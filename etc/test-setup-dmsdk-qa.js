...
function createAxis(manager, name) {
  return manager.post({
    endpoint: '/manage/v2/databases/'+testconfig.dmsdktestServerName+'/temporal/axes',
    body: {
      'axis-name': name+'Time',
      'axis-start': {
        'element-reference': {
...
function setup(manager) {
  ...
  result(function(response) {
    if (response.statusCode === 404) {
      console.log('creating database and REST server for '+testconfig.dmsdktestServerName);
      manager.post({
        endpoint: '/v1/rest-apis',
        body: {
          'rest-api': {
            name:               testconfig.dmsdktestServerName,
            group:              'Default',
      ...
      result(function(response) {
        if (response.statusCode === 201) {
          console.log('getting default indexes for '+testconfig.dmsdktestServerName);
          manager.get({
          ...
          then(function(response) {
            ...
            var body = {
                ...
                'triple-index':         true,
                'schema-database':      testconfig.dmsdktestServerName+'-modules',
				'merge-timestamp':      -6000000000,
                'range-path-index': [
                  {
                    'scalar-type': 'decimal',
            ...
            console.log('adding custom indexes for '+testconfig.dmsdktestServerName);
            return manager.put({
              endpoint: '/manage/v2/databases/'+testconfig.dmsdktestServerName+'/properties',
              params:   {
                format: 'json'
          ...
          then(function(response) {
            return manager.get({
              endpoint: '/manage/v2/databases/'+testconfig.dmsdktestServerName+'/temporal/axes/systemTime'
              }).result();
          }).
          then(function(response) {
          ...
          then(function(response) {
            return manager.get({
              endpoint: '/manage/v2/databases/'+testconfig.dmsdktestServerName+'/temporal/axes/validTime'
              }).result();
          }).
          then(function(response) {
            if (response.statusCode < 400) {
          ...
          then(function(response) {
            ...
            return manager.post({
              endpoint: '/manage/v2/databases/'+testconfig.dmsdktestServerName+'/temporal/collections',
              body: {
          ...
          then(function(response) {
            return manager.put({
              endpoint: '/manage/v2/databases/'+testconfig.dmsdktestServerName+'/temporal/collections/lsqt/properties?collection=temporalCollectionLsqt',
              body: {
                "lsqt-enabled": true,
                "automation": {
                  "enabled": true
          ...
          then(function(response) {
            console.log(testconfig.dmsdktestServerName+' setup succeeded');
            });
        } else {
          console.log(testconfig.dmsdktestServerName+' setup failed with HTTP status: '+response.statusCode);
          console.log(response.data);
        }
      });
    } else {
      console.log(testconfig.dmsdktestServerName+' test server is available on port '+
