...
class PlanPatchBuilderPlan extends types.ServerType {
...
subtract(...args) {
    ...
    * Add an error-handler to the Optic Pipeline to catch Optic Update runtime errors. The runtime errors are added in the errors column. If no error occurred the value of the error column is null. When added, the error-handler should be the last operator before op:result. Provides a client interface to a server function. See {@link http://docs.marklogic.com/Builder.prototype.onError|Builder.prototype.onError}
    * @method planBuilder#onError
    * @since 2.1.1
    * @param { XsString } [action] - The Optic Plan. You can either use the XQuery =&gt; chaining operator or specify the variable that captures the return value from the previous operation.
    * @param { PlanExprColName } [errorColumn] - Valid options are: "fail" - stop procesisng and "continue" - add an error to the error column and continue processing.
    * @returns { planBuilder.ModifyPlan }
    */
onError(...args) {
    ...
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.onError', 1, new Set(['action', 'error-column']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.onError', 1, false, paramdefs, args);
    return new PlanModifyPlan(null, 'op', 'on-error', checkedArgs);

    }
/**
    * Builds a patch operation including a sequence of inserts, replaces, replace-inserts and deletes. Provides a client interface to a server function. See {@link http://docs.marklogic.com/Builder.prototype.patch|Builder.prototype.patch}
    * @method planBuilder#patch
    * @since 2.1.1
    * @param { PlanExprColName } [docColumn] - The Optic Plan. You can either use the XQuery =&gt; chaining operator or specify the variable that captures the return value from the previous operation.
    * @param { PlanPatchBuilderPlan } [patchDef] - The document column which need to be patched.
    * @returns { planBuilder.ModifyPlan }
    */
patch(...args) {
    const namer = bldrbase.getNamer(args, 'doc-column');
    const paramdefs = [['doc-column', [PlanExprCol, PlanColumn, types.XsString], true, false], ['patch-def', [PlanPatchBuilderPlan], true, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.patch', 2, new Set(['doc-column', 'patch-def']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.patch', 2, false, paramdefs, args);
    return new PlanModifyPlan(null, 'op', 'patch', checkedArgs);
...
/**
    ...
    * @returns { planBuilder.PlanPatchBuilderPlan }
    */
patchBuilder(...args) {
    ...
    return new PlanPatchBuilderPlan('op', 'patch-builder', checkedArgs);
...
/**
    ...
    * @method planBuilder#insertAfter
    * @since 2.1.1
    ...
    * @param { Node } [node] - The path to insert the node after.
    * @returns { planBuilder.PlanPatchBuilderPlan }
    */
insertAfter(...args) {
    const namer = bldrbase.getNamer(args, 'path');
    const paramdefs = [['path', [types.XsString], true, false], ['node', [types.Node], true, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.insertAfter', 2, new Set(['path', 'node']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.insertAfter', 2, false, paramdefs, args);
    return new PlanPatchBuilderPlan('op', 'insert-after', checkedArgs);
...
/**
    ...
    * @method planBuilder#insertBefore
    * @since 2.1.1
    * @param { XsString } [path] - The Patch Builder Plan. You can either use the XQuery =&gt; chaining operator or specify the variable that captures the return value from the previous operation.
    * @param { Node } [node] - The path to insert the node before.
    * @returns { planBuilder.PlanPatchBuilderPlan }
    */
insertBefore(...args) {
    const namer = bldrbase.getNamer(args, 'path');
    const paramdefs = [['path', [types.XsString], true, false], ['node', [types.Node], true, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.insertBefore', 2, new Set(['path', 'node']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.insertBefore', 2, false, paramdefs, args);
    return new PlanPatchBuilderPlan('op', 'insert-before', checkedArgs);
...
/**
    ...
    * @method planBuilder#insertChild
    * @since 2.1.1
    * @param { XsString } [path] - The Patch Builder Plan. You can either use the XQuery =&gt; chaining operator or specify the variable that captures the return value from the previous operation.
    * @param { Node } [node] - The path to insert the child.
    * @returns { planBuilder.PlanPatchBuilderPlan }
    */
insertChild(...args) {
    const namer = bldrbase.getNamer(args, 'path');
    const paramdefs = [['path', [types.XsString], true, false], ['node', [types.Node], true, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.insertChild', 2, new Set(['path', 'node']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.insertChild', 2, false, paramdefs, args);
    return new PlanPatchBuilderPlan('op', 'insert-child', checkedArgs);
...
/**
    ...
    * @method planBuilder#insertNamedChild
    * @since 2.1.1
    * @param { XsString } [path] - The Patch Builder Plan. You can either use the XQuery =&gt; chaining operator or specify the variable that captures the return value from the previous operation.
    * @param { XsString } [key] - The path which returns an JSON Object.
    * @param { Node } [node] - The key to insert.
    * @returns { planBuilder.PlanPatchBuilderPlan }
    */
insertNamedChild(...args) {
    const namer = bldrbase.getNamer(args, 'path');
    const paramdefs = [['path', [types.XsString], true, false], ['key', [types.XsString], true, false], ['node', [types.Node], true, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.insertNamedChild', 3, new Set(['path', 'key', 'node']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.insertNamedChild', 3, false, paramdefs, args);
    return new PlanPatchBuilderPlan('op', 'insert-named-child', checkedArgs);
...
remove(...args) {
    ...
    const checkedArgs = bldrbase.makeSingleArgs('PlanBuilder.remove', 1, paramdef, args);
    return new PlanPatchBuilderPlan('op', 'remove', checkedArgs);
    }
/**
    ...
    * @method planBuilder#replace
    * @since 2.1.1
    * @param { XsString } [path] - The Patch Builder Plan. You can either use the XQuery =&gt; chaining operator or specify the variable that captures the return value from the previous operation.
    * @param { Node } [node] - The path to replace.
    * @returns { planBuilder.PlanPatchBuilderPlan }
    */
replace(...args) {
    const namer = bldrbase.getNamer(args, 'path');
    const paramdefs = [['path', [types.XsString], true, false], ['node', [types.Node], true, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.replace', 2, new Set(['path', 'node']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.replace', 2, false, paramdefs, args);
    return new PlanPatchBuilderPlan('op', 'replace', checkedArgs);
...
/**
    ...
    * @method planBuilder#replaceInsertChild
    * @since 2.1.1
    ...
    * @param { Node } [node] - The path to insert/replace which is relative to parent-path.
    * @returns { planBuilder.PlanPatchBuilderPlan }
    */
replaceInsertChild(...args) {
    const namer = bldrbase.getNamer(args, 'parent-path');
    const paramdefs = [['parent-path', [types.XsString], true, false], ['path-to-replace', [types.XsString], true, false], ['node', [types.Node], true, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.replaceInsertChild', 2, new Set(['parent-path', 'path-to-replace', 'node']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.replaceInsertChild', 2, false, paramdefs, args);
    return new PlanPatchBuilderPlan('op', 'replace-insert-child', checkedArgs);

    }
/**
    *  Provides a client interface to a server function.
    * @method planBuilder#replaceValue
    * @since 2.1.1
    * @param { XsString } [path] - 
    * @param { Item } [value] - 
    * @returns { planBuilder.PlanPatchBuilderPlan }
...
replaceValue(...args) {
    const namer = bldrbase.getNamer(args, 'path');
    const paramdefs = [['path', [types.XsString], true, false], ['value', [types.Item], true, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.replaceValue', 2, new Set(['path', 'value']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.replaceValue', 2, false, paramdefs, args);
    return new PlanPatchBuilderPlan('op', 'replace-value', checkedArgs);
