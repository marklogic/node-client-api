...
/**
    ...
    * @param { XsAnyAtomicType } [text] - The values to match. If multiple values are specified, the query matches if any of the values match (or-query semantics). For XML and metadata, the values should be strings. For JSON, values can be strings, numbers or booleans to match correspondingly typed nodes. To match null, pass in the empty sequence.
...
/**
    ...
    * @param { XsAnyAtomicType } [zero] - The value to return as zero if the input sequence is the empty sequence.
    * @returns { XsAnyAtomicType }
    */
sum(...args) {
    const namer = bldrbase.getNamer(args, 'arg');
...
/**
    * Returns an integer specifying the index of the bucket the second parameter belongs to in buckets formed by the first parameter. Values that lie on the edge of a bucket fall to the lesser index. Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.bucket|sql.bucket}
    * @method planBuilder.sql#bucket
    * @since 2.1.1
    * @param { XsAnyAtomicType } [bucketEdgesParam] - A sequence of ordered values indicating the edges of a collection of buckets.
