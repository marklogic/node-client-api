...
complexPolygon(...args) {
    ...
    const paramdefs = [['outer', [types.CtsPolygon], true, false], ['inner', [types.CtsPolygon], false, true]];
    const checkedArgs = (namer !== null) ?
...
ellipsePolygon(...args) {
    ...
    const paramdefs = [['center', [types.CtsPoint, PlanColumn], false, true], ['semi-major-axis', [types.XsDouble, PlanColumn, PlanParam], true, false], ['semi-minor-axis', [types.XsDouble, PlanColumn, PlanParam], true, false], ['azimuth', [types.XsDouble, PlanColumn, PlanParam], true, false], ['arc-tolerance', [types.XsDouble, PlanColumn, PlanParam], true, false], ['options', [types.XsString, PlanColumn, PlanParam], false, true]];
    const checkedArgs = (namer !== null) ?
...
regionAffineTransform(...args) {
    const namer = bldrbase.getNamer(args, 'region');
    const paramdefs = [['region', [types.CtsRegion, PlanColumn], true, false], ['transform', [types.MapMap, PlanColumn], false, true], ['options', [types.XsString], false, true]];
    const checkedArgs = (namer !== null) ?
...
regionApproximate(...args) {
    const namer = bldrbase.getNamer(args, 'region');
    const paramdefs = [['region', [types.CtsRegion, PlanColumn], true, false], ['threshold', [types.XsDouble, PlanColumn], true, false], ['options', [types.XsString, PlanColumn, PlanParam], false, true]];
    const checkedArgs = (namer !== null) ?
...
regionClean(...args) {
    const namer = bldrbase.getNamer(args, 'region');
    const paramdefs = [['region', [types.CtsRegion, PlanColumn], true, false], ['options', [types.XsString, PlanColumn, PlanParam], false, true]];
    const checkedArgs = (namer !== null) ?
...
/**
    * Returns an unsignedLong specifying the index of the bucket the second parameter belongs to in buckets formed by the first parameter. Values that lie on the edge of a bucket fall to the lesser index. Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.bucket|sql.bucket}
...
/**
    ...
    * @param { XsString } [escape] - If a '_' ann '%' are preceeded by an escape character then it will be match as the char '-'/'%' themselves.
    * @returns { XsBoolean }
    */
like(...args) {
    const namer = bldrbase.getNamer(args, 'input');
...
facetBy(...args) {
    ...
    const paramdefs = [['keys', [PlanExprCol, PlanColumn, types.XsString], true, true], ['count-col', [PlanExprCol, PlanColumn, types.XsString], false, false]];
    const checkedArgs = (namer !== null) ?
...
/**
    ...
    * @since 2.1.1
    * @param { XsAnyAtomicType } [left] - Two or more expressions.
    * @param { XsAnyAtomicType } [right] - Two or more expressions.
    * @returns { XsBoolean }
    */
eq(...args) {
        const namer = bldrbase.getNamer(args, 'left');
...
/**
    ...
    * @since 2.1.1
    * @param { XsAnyAtomicType } [left] - The expression providing the value to test.
    * @param { XsAnyAtomicType } [right] - One or more expressions providing the candidate values.
    * @returns { XsBoolean }
    */
in(...args) {
        const namer = bldrbase.getNamer(args, 'left');
        const paramdefs = [['left', [types.XsAnyAtomicType, PlanColumn, PlanParam], true, false], ['right', [types.XsAnyAtomicType, PlanColumn, PlanParam], true, true]];
        if (namer !== null) {
            const paramNames = new Set(['left', 'right']);
...
/**
    ...
    * @param { PlanExprColName } [keys] - The identifier for the existing column with the values (typically numeric or datetime) to put into buckets. The columns can be named with a string or a column parameter function such as op:col or constructed from an expression with op:as.
...
bucketGroup(...args) {
    const namer = bldrbase.getNamer(args, 'name');
    const paramdefs = [['name', [types.XsString], true, false], ['keys', [PlanExprCol, PlanColumn, types.XsString], true, false], ['boundaries', [types.XsAnyAtomicType], true, true], ['collation', [types.XsString], false, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.bucketGroup', 3, new Set(['name', 'keys', 'boundaries', 'collation']), paramdefs, args) :
