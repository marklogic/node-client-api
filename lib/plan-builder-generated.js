...
/**
    ...
    * @since 3.5.0
    * @param { XsAnyAtomicType } [values] - The value(s) to create the vector from. Can be a sequence or json:array of integer or floating-point numbers. Also accepts a string that has the format of a JSON array of Numbers, a string that was created by vec:base64-encode(), or a types.Node such as returned by xpath.
    * @returns { VecVector }
    */
vector(...args) {
    const paramdef = ['values', [types.XsAnyAtomicType, types.Node, PlanColumn, PlanParam], false, true];
...
class PlanAnnTopKOptions extends types.ServerType {
...
/**
  * ColumnBuilder object to construct a view for op.fromDocs. 
  * This class has been modified from the auto-generated classes to 
  * add chaining behavior to the methods to collect multiple method 
  * calls into a single ColumnBuilder object to send to /v1/rows.
  * This is done by returning a new PlanColumnBuilderBase and follows
  * the PatchBuilder pattern.
  * @namespace planBuilder.ColumnBuilder
  * @since 4.1.0
  */
class PlanColumnBuilder extends types.ServerType {
...
/**
    ...
    */
addColumn(...args) {
    const paramdef = ['column', [PlanColumn, types.XsString], true, false];
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.addColumn', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'add-column', checkedArgs)]);
    }
/**
    * Sets the collation for the column created by op:add-column. This only applies to columns with string types, as collations specify the order in which strings are sorted and how they are compared. Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.collation|op.collation}
    * @method planBuilder.ColumnBuilder#collation
    * @since 4.1.0
    * @param { XsString } [collation] - the collation for string types
    * @returns { planBuilder.ColumnBuilder }
    */
collation(...args) {
    const paramdef = ['collation', [types.XsString], true, false];
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.collation', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'collation', checkedArgs)]);
    }
/**
    * Sets the coordinate-system for the column created by op:add-column. This only applies to columns with geo types. Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.coordinateSystem|op.coordinateSystem}
    * @method planBuilder.ColumnBuilder#coordinateSystem
    * @since 4.1.0
    * @param { XsString } [coordinateSystem] - the coordinate system for geo types
    * @returns { planBuilder.ColumnBuilder }
    */
coordinateSystem(...args) {
    const paramdef = ['coordinate-system', [types.XsString], true, false];
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.coordinateSystem', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'coordinate-system', checkedArgs)]);
    }
/**
    * Sets the default value for the column created by op:add-column. Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.default|op.default}
    * @method planBuilder.ColumnBuilder#default
    * @since 4.1.0
    * @param { Item } [defaultExpression] - the default value for the column
    * @returns { planBuilder.ColumnBuilder }
    */
default(...args) {
    const paramdef = ['default-expression', [types.Item, PlanColumn, PlanParam], true, false];
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.default', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'default', checkedArgs)]);
    }
/**
    * Sets the vector dimension for the column created by op:add-column. It only applies when the column's type is set to "vector". If the vector to be extracted does not have the same dimension, the value will be rejected. There's no default dimension. Vectors of all dimensions are extracted. Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.dimension|op.dimension}
    * @method planBuilder.ColumnBuilder#dimension
    * @since 4.1.0
    * @param { XsUnsignedInt } [dimension] - the vector dimension for the column
    * @returns { planBuilder.ColumnBuilder }
    */
dimension(...args) {
    const paramdef = ['dimension', [types.XsUnsignedInt], true, false];
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.dimension', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'dimension', checkedArgs)]);
    }
/**
    * Sets the expression to create content or extract content from a document for the column created by op:add-column. It cannot be used together with op:xpath, as both of them define the content of the column. Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.expr|op.expr}
    * @method planBuilder.ColumnBuilder#expr
    * @since 4.1.0
    * @param { Item } [expression] - the expression to create content or extract content from a document for the column
    * @returns { planBuilder.ColumnBuilder }
    */
expr(...args) {
    const paramdef = ['expression', [types.Item, PlanColumn, PlanParam], true, false];
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.expr', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'expr', checkedArgs)]);
    }
/**
    * Set the column created by op:add-column nullable or not. Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.nullable|op.nullable}
    * @method planBuilder.ColumnBuilder#nullable
    * @since 4.1.0
    * @param { XsBoolean } [bool] - whether the column is nullable
    * @returns { planBuilder.ColumnBuilder }
    */
nullable(...args) {
    const paramdef = ['bool', [types.XsBoolean], true, false];
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.nullable', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'nullable', checkedArgs)]);
    }
/**
    * Set the data type of the column created by op:add-column. Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.type|op.type}
    * @method planBuilder.ColumnBuilder#type
    * @since 4.1.0
    * @param { XsString } [type] - the data type of the column
    * @returns { planBuilder.ColumnBuilder }
    */
type(...args) {
    const paramdef = ['type', [types.XsString], true, false];
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.type', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'type', checkedArgs)]);
    }
/**
    * Sets the units for the column created by op:add-column. This only applies to columns with geo types. Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.units|op.units}
    * @method planBuilder.ColumnBuilder#units
    * @since 4.1.0
    * @param { XsString } [units] - the units for the column
    * @returns { planBuilder.ColumnBuilder }
    */
units(...args) {
    const paramdef = ['units', [types.XsString], true, false];
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.units', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'units', checkedArgs)]);
    }
/**
...
xpath(...args) {
    ...
    const checkedArgs = bldrbase.makeSingleArgs('PlanColumnBuilder.xpath', 1, paramdef, args);
    return new PlanColumnBuilderBase([new PlanColumnBuilder('op', 'xpath', checkedArgs)]);
    }
}

/**
 * Helper class to collect column builder operations.
 * @namespace planBuilder.PlanColumnBuilderBase
 * @since 4.1.0
 */
class PlanColumnBuilderBase {
    constructor(args) {
        columnBuilderArgs.push(args);
        return new PlanColumnBuilder('op', 'suboperators', columnBuilderArgs);
    }
}
...
class PlanContextExprCall extends types.ServerType {
...
/**
    ...
    * @since 3.6.0
    * @param { PlanInputKParam } [inputK] - This positive integer k is the number of nearest neighbour results to return.
    * @param { PlanExprColName } [vectorColumn] - The column is the input table to perform vector distance calculations against. The columns can be named with a string or a column function such as op:col, op:view-col, or op:schema-col.
    * @param { PlanQueryVectorParam } [queryVector] - This is the query vector to compare with the vectors from the vector-column column.
    * @param { PlanExprColName } [distanceColumn] - The column is an optional output column that returns the value of the distance metric for that output row. The columns can be named with a string or a column function such as op:col, op:view-col, or op:schema-col.
    * @param { PlanAnnTopKOptions } [options] - This is either a sequence of strings or a map containing keys and values for the options to this operator. Options include: max-distance This option is a number determines the maximum distance for a returned result. For cosine distance, the default is float max. Rows with a distance greater than this will not be returned. search-factor This option can be used to increase or decrease the number of candidate vectors found from the index, and defaults to 1.0. Higher values will result in slower searches that may provide higher results accuracy. Lower values will result in faster searches that may give lower accuracy.distance Takes values of cosine for now, defaulting to cosine.
    * @returns { planBuilder.ModifyPlan }
    */
annTopK(...args) {
...
/**
    ...
    */
fromDocs(...args) {
    const namer = bldrbase.getNamer(args, 'cts-query');
    const paramdefs = [['cts-query', [types.CtsQuery, types.XsString], true, false], ['context-path', [types.XsString], true, false], ['column-spec', [PlanColumnBuilder], true, false], ['qualifier', [types.XsString], false, false], ['system-col', [PlanSystemColumn], false, false], ['namespaces', [PlanNamespaceBindings], false, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'PlanBuilder.fromDocs', 3, new Set(['cts-query', 'context-path', 'column-spec', 'qualifier', 'system-col', 'namespaces']), paramdefs, args) :
        bldrbase.makePositionalArgs('PlanBuilder.fromDocs', 3, false, paramdefs, args);
    return new PlanAccessPlan(null, 'op', 'from-docs', checkedArgs);
...
/**
    ...
    * @since 2.1.1
    * @param { PlanXpathExprColName } [column] - The name of the column from which to extract the child nodes. The column can be named with a string or a column function such as op:col, op:view-col, or op:schema-col, or constructed from an expression with the op:as function.It can also be op:context to refer to the node of the current processing row.
...
xpath(...args) {
    const namer = bldrbase.getNamer(args, 'column');
    const paramdefs = [['column', [PlanExprCol, PlanColumn, types.XsString, PlanContextExprCall], true, false], ['path', [types.XsString, PlanColumn, PlanParam], true, false], ['namespace-bindings', [PlanNamespaceBindings], false, true]];
    const checkedArgs = (namer !== null) ?
...
/**
    * This helper function returns the node from the current processing row. It is to be used in op:xpath, to reference the 'current item' instead of a doc column. Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.context|op.context}
    * @method planBuilder#context
    * @since 4.1.0

    * @returns { planBuilder.PlanContextExprCall }
    */
context(...args) {
    bldrbase.checkMaxArity('PlanBuilder.context', args.length, 0);
    return new PlanContextExprCall('op', 'context', args);
    }
/**
