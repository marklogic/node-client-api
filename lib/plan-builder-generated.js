...
/**
    * <p>Adjusts an xs:date value to a specific timezone, or to no timezone at all. If $timezone is the empty sequence, returns an xs:date without a timezone. Otherwise, returns an xs:date with a timezone. For purposes of timezone adjustment, an xs:date is treated as an xs:dateTime with time 00:00:00. </p><p> If $timezone is not specified, then $timezone is the value of the implicit timezone in the dynamic context. </p><p> If $arg is the empty sequence, then the result is the empty sequence. </p><p> A dynamic error is raised [err:FODT0003] if $timezone is less than -PT14H or greater than PT14H or if does not contain an integral number of minutes. </p><p> If $arg does not have a timezone component and $timezone is the empty sequence, then the result is $arg. </p><p> If $arg does not have a timezone component and $timezone is not the empty sequence, then the result is $arg with $timezone as the timezone component. </p><p> If $arg has a timezone component and $timezone is the empty sequence, then the result is the localized value of $arg without its timezone component. </p><p> If $arg has a timezone component and $timezone is not the empty sequence, then: </p><p> Let $srcdt be an xs:dateTime value, with 00:00:00 for the time component and date and timezone components that are the same as the date and timezone components of $arg. </p><p> Let $r be the result of evaluating fn:adjust-dateTime-to-timezone($srcdt, $timezone) </p><p> The result of this function will be a date value that has date and timezone components that are the same as the date and timezone components of $r. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.adjustDateToTimezone|fn.adjustDateToTimezone}
...
/**
    * <p>Adjusts an xs:dateTime value to a specific timezone, or to no timezone at all. If $timezone is the empty sequence, returns an xs:dateTime without a timezone. Otherwise, returns an xs:dateTime with a timezone. </p><p> If $timezone is not specified, then $timezone is the value of the implicit timezone in the dynamic context. </p><p> If $arg is the empty sequence, then the result is the empty sequence. </p><p> A dynamic error is raised [err:FODT0003] if $timezone is less than -PT14H or greater than PT14H or if does not contain an integral number of minutes. </p><p> If $arg does not have a timezone component and $timezone is the empty sequence, then the result is $arg. </p><p> If $arg does not have a timezone component and $timezone is not the empty sequence, then the result is $arg with $timezone as the timezone component. </p><p> If $arg has a timezone component and $timezone is the empty sequence, then the result is the localized value of $arg without its timezone component. </p><p> If $arg has a timezone component and $timezone is not the empty sequence, then the result is an xs:dateTime value with a timezone component of $timezone that is equal to $arg. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.adjustDateTimeToTimezone|fn.adjustDateTimeToTimezone}
...
/**
    * <p>Adjusts an xs:time value to a specific timezone, or to no timezone at all. If $timezone is the empty sequence, returns an xs:time without a timezone. Otherwise, returns an xs:time with a timezone. </p><p> If $timezone is not specified, then $timezone is the value of the implicit timezone in the dynamic context. </p><p> If $arg is the empty sequence, then the result is the empty sequence. </p><p> A dynamic error is raised [err:FODT0003] if $timezone is less than -PT14H or greater than PT14H or if does not contain an integral number of minutes. </p><p> If $arg does not have a timezone component and $timezone is the empty sequence, then the result is $arg. </p><p> If $arg does not have a timezone component and $timezone is not the empty sequence, then the result is $arg with $timezone as the timezone component. </p><p> If $arg has a timezone component and $timezone is the empty sequence, then the result is the localized value of $arg without its timezone component. </p><p> If $arg has a timezone component and $timezone is not the empty sequence, then: </p><p> Let $srcdt be an xs:dateTime value, with an arbitrary date for the date component and time and timezone components that are the same as the time and timezone components of $arg. </p><p> Let $r be the result of evaluating fn:adjust-dateTime-to-timezone($srcdt, $timezone) </p><p> The result of this function will be a time value that has time and timezone components that are the same as the time and timezone components of $r. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.adjustTimeToTimezone|fn.adjustTimeToTimezone}
...
/**
    * <p> Returns the average of the values in the input sequence $arg, that is, the sum of the values divided by the number of values. </p><p> If $arg is the empty sequence, the empty sequence is returned. </p><p> If $arg contains values of type xs:untypedAtomic they are cast to xs:double. </p><p> Duration values must either all be xs:yearMonthDuration values or must all be xs:dayTimeDuration values. For numeric values, the numeric promotion rules defined in 6.2 Operators on Numeric Values are used to promote all values to a single common type. After these operations, $arg must contain items of a single type, which must be one of the four numeric types,xs:yearMonthDuration or xs:dayTimeDuration or one if its subtypes. </p><p> If the above conditions are not met, then a type error is raised [err:FORG0006]. </p><p> Otherwise, returns the average of the values computed as sum($arg) div count($arg). </p><p> For detailed type semantics, see <a>Section 7.2.10 The fn:min, fn:max, fn:avg, and fn:sum functions[FS]</a>. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.avg|fn.avg}
...
/**
    * <p> Computes the effective boolean value of the sequence $arg. See Section 2.4.3 Effective Boolean Value[XP]. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.boolean|fn.boolean}
...
/**
    * <p> Returns <code>true</code> if the specified parameters are the same Unicode code point, otherwise returns <code>false</code>. The codepoints are compared according to the Unicode code point collation (<a>http://www.w3.org/2005/xpath-functions/collation/codepoint</a>). </p> <p> If either argument is the empty sequence, the result is the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.codepointEqual|fn.codepointEqual}
...
/**
    * <p> Returns -1, 0, or 1, depending on whether the value of the $comparand1 is respectively less than, equal to, or greater than the value of $comparand2, according to the rules of the collation that is used. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.compare|fn.compare}
...
/**
    * <p> Returns the <code>xs:string</code> that is the concatenation of the values of the specified parameters. Accepts two or more <code>xs:anyAtomicType</code> arguments and casts them to <code>xs:string</code>. If any of the parameters is the empty sequence, the parameter is treated as the zero-length string. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.concat|fn.concat}
...
/**
    * <p>Returns the number of items in the value of $arg. </p><p> Returns 0 if $arg is the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.count|fn.count}
...
/**
    * <p>Returns an xs:integer between 1 and 31, both inclusive, representing the day component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.dayFromDate|fn.dayFromDate}
...
/**
    * <p> Returns an xs:integer between 1 and 31, both inclusive, representing the day component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.dayFromDateTime|fn.dayFromDateTime}
...
/**
    * <p> Returns an xs:integer representing the days component in the canonical lexical representation of the value of $arg. The result may be negative. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.daysFromDuration|fn.daysFromDuration}
...
/**
    * <p> Returns the sequence that results from removing from $arg all but one of a set of values that are eq to one other. Values that cannot be compared, i.e. the eq operator is not defined for their types, are considered to be distinct. Values of type xs:untypedAtomic are compared as if they were of type xs:string. The order in which the sequence of values is returned is implementation dependent. </p><p> The static type of the result is a sequence of prime types as defined in <a>Section 7.2.7 The fn:distinct-values function[FS]</a>. </p><p> The collation used by the invocation of this function is determined according to the rules in 7.3.1 Collations. The collation is used when string comparison is required. </p><p> If $arg is the empty sequence, the empty sequence is returned. </p><p> For xs:float and xs:double values, positive zero is equal to negative zero and, although NaN does not equal itself, if $arg contains multiple NaN values a single NaN is returned. </p><p> If xs:dateTime, xs:date or xs:time values do not have a timezone, they are considered to have the implicit timezone provided by the dynamic context for the purpose of comparison. Note that xs:dateTime, xs:date or xs:time values can compare equal even if their timezones are different. </p><p> Which value of a set of values that compare equal is returned is implementation dependent. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.distinctValues|fn.distinctValues}
...
/**
    * <p>Returns a string that uniquely identifies a given node. </p> <p> If $node is the empty sequence, the zero-length string is returned. </p> <p> If the function is called without an argument, the context item is used as the default argument. The behavior of the function when the argument is omitted is the same as if the context item is passed as an argument.</p> <p>If the context item is undefined an error is raised: [err:XPDY0002]. If the context item is not a node an error is raised: [err:XPTY0004]. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.generateId|fn.generateId}
...
/**
    * <p> Returns an xs:integer between 0 and 23, both inclusive, representing the hours component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.hoursFromDateTime|fn.hoursFromDateTime}
...
/**
    * <p>Returns an xs:integer representing the hours component in the canonical lexical representation of the value of $arg. The result may be negative. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.hoursFromDuration|fn.hoursFromDuration}
...
/**
    * <p> Returns an xs:integer between 0 and 23, both inclusive, representing the value of the hours component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.hoursFromTime|fn.hoursFromTime}
...
/**
    * <p> Returns the prefixes of the in-scope namespaces for $element. For namespaces that have a prefix, it returns the prefix as an xs:NCName. For the default namespace, which has no prefix, it returns the zero-length string. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.inScopePrefixes|fn.inScopePrefixes}
...
/**
    * <p> Returns a sequence of positive integers giving the positions within the sequence $seqParam of items that are equal to $srchParam. </p><p> The collation used by the invocation of this function is determined according to the rules in 7.3.1 Collations. The collation is used when string comparison is required. </p><p> The items in the sequence $seqParam are compared with $srchParam under the rules for the eq operator. Values that cannot be compared, i.e. the eq operator is not defined for their types, are considered to be distinct. If an item compares equal, then the position of that item in the sequence $srchParam is included in the result. </p><p> If the value of $seqParam is the empty sequence, or if no item in $seqParam matches $srchParam, then the empty sequence is returned. </p><p> The first item in a sequence is at position 1, not position 0. </p><p> The result sequence is in ascending numeric order. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.indexOf|fn.indexOf}
...
/**
    * <p> Returns a new sequence constructed from the value of $target with the value of $inserts inserted at the position specified by the value of $position. (The value of $target is not affected by the sequence construction.) </p><p> If $target is the empty sequence, $inserts is returned. If $inserts is the empty sequence, $target is returned. </p><p> The value returned by the function consists of all items of $target whose index is less than $position, followed by all items of $inserts, followed by the remaining elements of $target, in that sequence. </p><p> If $position is less than one (1), the first position, the effective value of $position is one (1). If $position is greater than the number of items in $target, then the effective value of $position is equal to the number of items in $target plus 1. </p><p> For detailed semantics see, <a>Section 7.2.15 The fn:insert-before function[FS]</a>. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.insertBefore|fn.insertBefore}
...
/**
    * <p>Returns the local part of the name of $arg as an xs:string that will either be the zero-length string or will have the lexical form of an xs:NCName. </p><p> If the argument is omitted, it defaults to the context node. If the context item is undefined an error is raised: [err:XPDY0002]. If the context item is not a node an error is raised: [err:XPTY0004]. </p><p> If the argument is supplied and is the empty sequence, the function returns the zero-length string. </p><p> If the target node has no name (that is, if it is a document node, a comment, a text node, or a namespace node having no name), the function returns the zero-length string. </p><p> Otherwise, the value returned will be the local part of the expanded-QName of the target node (as determined by the dm:node-name accessor in Section 5.11 node-name Accessor[DM]. This will be an xs:string whose lexical form is an xs:NCName. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.localName|fn.localName}
...
/**
    * <p> Selects an item from the input sequence $arg whose value is greater than or equal to the value of every other item in the input sequence. If there are two or more such items, then the specific item whose value is returned is implementation dependent. </p><p> The following rules are applied to the input sequence: </p> <ul><li>Values of type xs:untypedAtomic in $arg are cast to xs:double.</li><li>For numeric values, the numeric promotion rules defined in 6.2 Operators on Numeric Values are used to promote all values to a single common type. </li></ul> <p> The items in the resulting sequence may be reordered in an arbitrary order. The resulting sequence is referred to below as the converted sequence.This function returns an item from the converted sequence rather than the input sequence. </p><p> If the converted sequence is empty, the empty sequence is returned. </p><p> All items in $arg must be numeric or derived from a single base type for which the ge operator is defined. In addition, the values in the sequence must have a total order. If date/time values do not have a timezone, they are considered to have the implicit timezone provided by the dynamic context for purposes of comparison. Duration values must either all be xs:yearMonthDuration values or must all be xs:dayTimeDuration values. </p><p> If any of these conditions is not met, then a type error is raised [err:FORG0006]. </p><p> If the converted sequence contains the value NaN, the value NaN is returned. </p><p> If the items in the value of $arg are of type xs:string or types derived by restriction from xs:string, then the determination of the item with the largest value is made according to the collation that is used. If the type of the items in $arg is not xs:string and $collation is specified, the collation is ignored. </p><p> The collation used by the invocation of this function is determined according to the rules in 7.3.1 Collations. </p><p> Otherwise, the result of the function is the result of the expression: </p><pre> if (every $v in $c satisfies $c[1] ge $v) then $c[1] else fn:max(fn:subsequence($c, 2)) </pre> <p> evaluated with $collation as the default collation if specified, and with $c as the converted sequence. </p><p> For detailed type semantics, see <a>Section 7.2.10 The fn:min, fn:max, fn:avg, and fn:sum functions[FS]</a>. </p><p> Notes: </p><p> If the converted sequence contains exactly one value then that value is returned. </p><p> The default type when the fn:max function is applied to xs:untypedAtomic values is xs:double. This differs from the default type for operators such as gt, and for sorting in XQuery and XSLT, which is xs:string. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.max|fn.max}
...
/**
    * <p> Selects an item from the input sequence $arg whose value is less than or equal to the value of every other item in the input sequence. If there are two or more such items, then the specific item whose value is returned is implementation dependent. </p><p> The following rules are applied to the input sequence: </p> <ul><li>Values of type xs:untypedAtomic in $arg are cast to xs:double.</li><li>For numeric values, the numeric promotion rules defined in 6.2 Operators on Numeric Values are used to promote all values to a single common type. </li></ul> <p> The items in the resulting sequence may be reordered in an arbitrary order. The resulting sequence is referred to below as the converted sequence.This function returns an item from the converted sequence rather than the input sequence. </p><p> If the converted sequence is empty, the empty sequence is returned. </p><p> All items in $arg must be numeric or derived from a single base type for which the le operator is defined. In addition, the values in the sequence must have a total order. If date/time values do not have a timezone, they are considered to have the implicit timezone provided by the dynamic context for purposes of comparison. Duration values must either all be xs:yearMonthDuration values or must all be xs:dayTimeDuration values. </p><p> If any of these conditions is not met, then a type error is raised [err:FORG0006]. </p><p> If the converted sequence contains the value NaN, the value NaN is returned. </p><p> If the items in the value of $arg are of type xs:string or types derived by restriction from xs:string, then the determination of the item with the largest value is made according to the collation that is used. If the type of the items in $arg is not xs:string and $collation is specified, the collation is ignored. </p><p> The collation used by the invocation of this function is determined according to the rules in 7.3.1 Collations. </p><p> Otherwise, the result of the function is the result of the expression: </p><pre> if (every $v in $c satisfies $c[1] le $v) then $c[1] else fn:min(fn:subsequence($c, 2)) </pre> <p> evaluated with $collation as the default collation if specified, and with $c as the converted sequence. </p><p> For detailed type semantics, see <a>Section 7.2.10 The fn:min, fn:max, fn:avg, and fn:sum functions[FS]</a>. </p><p> Notes: </p><p> If the converted sequence contains exactly one value then that value is returned. </p><p> The default type when the fn:min function is applied to xs:untypedAtomic values is xs:double. This differs from the default type for operators such as gt, and for sorting in XQuery and XSLT, which is xs:string. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.min|fn.min}
...
/**
    * <p>Returns an xs:integer value between 0 and 59, both inclusive, representing the minute component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.minutesFromDateTime|fn.minutesFromDateTime}
...
/**
    * <p>Returns an xs:integer representing the minutes component in the canonical lexical representation of the value of $arg. The result may be negative. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.minutesFromDuration|fn.minutesFromDuration}
...
/**
    * <p> Returns an xs:integer value between 0 to 59, both inclusive, representing the value of the minutes component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.minutesFromTime|fn.minutesFromTime}
...
/**
    * <p>Returns an xs:integer between 1 and 12, both inclusive, representing the month component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.monthFromDate|fn.monthFromDate}
...
/**
    * <p> Returns an xs:integer between 1 and 12, both inclusive, representing the month component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.monthFromDateTime|fn.monthFromDateTime}
...
/**
    * <p>Returns an xs:integer representing the months component in the canonical lexical representation of the value of $arg. The result may be negative. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.monthsFromDuration|fn.monthsFromDuration}
...
/**
    * <p>Returns the name of a node, as an <code>xs:string</code> that is either the zero-length string, or has the lexical form of an <code>xs:QName</code>. </p><p> If the argument is omitted, it defaults to the context node. If the context item is undefined an error is raised: [err:XPDY002]. If the context item is not a node an error is raised: [err:XPTY0004]. </p><p> If the argument is supplied and is the empty sequence, the function returns the zero-length string. </p><p> If the target node has no name (that is, if it is a document node, a comment, a text node, or a namespace node having no name), the function returns the zero-length string. </p> <p> If the specified node was created with a namespace prefix, that namespace prefix is returned with the element localname (for example, <code>a:hello</code>). Note that the namespace prefix is not always the same prefix that would be returned if you serialized the QName of the node, as the serialized QName will use the namespace from the XQuery context in which it was serialized.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.name|fn.name}
...
/**
    * <p>Returns the namespace URI of the xs:QName of the node specified by $arg. </p><p> If the argument is omitted, it defaults to the context node. If the context item is undefined an error is raised: [err:XPDY0002]. If the context item is not a node an error is raised: [err:XPTY0004]. </p><p> If $arg is the empty sequence, the xs:anyURI corresponding to the zero-length string is returned. </p><p> If $arg is neither an element nor an attribute node, or if it is an element or attribute node whose expanded-QName (as determined by the dm:node-name accessor in the Section 5.11 node-name Accessor[DM]) is in no namespace, then the function returns the xs:anyURI corresponding to the zero-length string. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.namespaceUri|fn.namespaceUri}
...
/**
    * <p> Summary: Returns an xs:boolean indicating whether the argument node is "nilled". If the argument is not an element node, returns the empty sequence. If the argument is the empty sequence, returns the empty sequence. For element nodes, true() is returned if the element is nilled, otherwise false().</p> <p> Elements may be defined in a schema as nillable, which allows an empty instance of an element to a appear in a document even though its type requires that it always have some content. Nilled elements should always be empty but an element is not considered nilled just because it's empty. It must also have the type annotation attribute xsi:nil="true". </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.nilled|fn.nilled}
...
/**
    * <p> Returns the value indicated by $arg or, if $arg is not specified, the context item after atomization, converted to an xs:double. If $arg is the empty sequence or if $arg or the context item cannot be converted to an xs:double, the xs:double value NaN is returned. If the context item is undefined an error is raised: [err:XPDY0002]. </p><p> Calling the zero-argument version of the function is defined to give the same result as calling the single-argument version with an argument of ".". That is, fn:number() is equivalent to fn:number(.). </p><p> If $arg is the empty sequence, NaN is returned. Otherwise, $arg, or the context item after atomization, is converted to an xs:double following the rules of 17.1.3.2 Casting to xs:double. If the conversion to xs:double fails, the xs:double value NaN is returned. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.number|fn.number}
...
/**
    * <p> Returns an <code>xs:QName</code> with the namespace URI given in $paramURI. If $paramURI is the zero-length string or the empty sequence, it represents "no namespace"; in this case, if the value of $paramQName contains a colon (:), an error is raised [err:FOCA0002]. The prefix (or absence of a prefix) in $paramQName is retained in the returned xs:QName value. The local name in the result is taken from the local part of $paramQName. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.QName|fn.QName}
...
/**
    * <p> Returns a new sequence constructed from the value of $target with the item at the position specified by the value of $position removed. </p><p> If $position is less than 1 or greater than the number of items in $target, $target is returned. Otherwise, the value returned by the function consists of all items of $target whose index is less than $position, followed by all items of $target whose index is greater than $position. If $target is the empty sequence, the empty sequence is returned. </p><p> For detailed type semantics, see <a>Section 7.2.11 The fn:remove function[FS]</a>. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.remove|fn.remove}
...
/**
    * <p>Reverses the order of items in a sequence. If $arg is the empty sequence, the empty sequence is returned. </p><p> For detailed type semantics, see <a>Section 7.2.12 The fn:reverse function[FS]</a>. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.reverse|fn.reverse}
...
/**
    * <p>Returns the root of the tree to which $arg belongs. This will usually, but not necessarily, be a document node. </p><p> If $arg is the empty sequence, the empty sequence is returned. </p><p> If $arg is a document node, $arg is returned. </p><p> If the function is called without an argument, the context item is used as the default argument. If the context item is undefined an error is raised: [err:XPDY0002]. If the context item is not a node an error is raised: [err:XPTY0004]. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.root|fn.root}
...
/**
    * <p>Returns an xs:decimal representing the seconds component in the canonical lexical representation of the value of $arg. The result may be negative. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.secondsFromDuration|fn.secondsFromDuration}
...
/**
    * <p> Returns an <code>xs:string</code> created by concatenating the members of the $parameter1 sequence using $parameter2 as a separator. If the value of $arg2 is the zero-length string, then the members of $parameter1 are concatenated without a separator. </p><p> If the value of $parameter1 is the empty sequence, the zero-length string is returned. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.stringJoin|fn.stringJoin}
...
/**
    * <p> Returns the sequence of Unicode code points that constitute an xs:string. If $arg is a zero-length string or the empty sequence, the empty sequence is returned. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.stringToCodepoints|fn.stringToCodepoints}
...
/**
    * <p> Returns the contiguous sequence of items in the value of $sourceSeq beginning at the position indicated by the value of $startingLoc and continuing for the number of items indicated by the value of $length. </p><p> In the two-argument case, returns: </p><p> $sourceSeq[fn:round($startingLoc) le $p] </p><p> In the three-argument case, returns: </p><p> $sourceSeq[fn:round($startingLoc) le $p and $p lt fn:round($startingLoc) + fn:round($length)] </p><p> Notes: </p><p> If $sourceSeq is the empty sequence, the empty sequence is returned. </p><p> If $startingLoc is zero or negative, the subsequence includes items from the beginning of the $sourceSeq. </p><p> If $length is not specified, the subsequence includes items to the end of $sourceSeq. </p><p> If $length is greater than the number of items in the value of $sourceSeq following $startingLoc, the subsequence includes items to the end of $sourceSeq. </p><p> The first item of a sequence is located at position 1, not position 0. </p><p> For detailed type semantics, see Section 7.2.13 The fn:subsequence functionFS. </p><p> The reason the function accepts arguments of type xs:double is that many computations on untyped data return an xs:double result; and the reason for the rounding rules is to compensate for any imprecision in these floating-point computations. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.subsequence|fn.subsequence}
...
/**
    * <p> Returns a value obtained by adding together the values in $arg. If $zero is not specified, then the value returned for an empty sequence is the xs:integer value 0. If $zero is specified, then the value returned for an empty sequence is $zero. </p><p> Any values of type xs:untypedAtomic in $arg are cast to xs:double. The items in the resulting sequence may be reordered in an arbitrary order. The resulting sequence is referred to below as the converted sequence. </p><p> If the converted sequence is empty, then the single-argument form of the function returns the xs:integer value 0; the two-argument form returns the value of the argument $zero. </p><p> If the converted sequence contains the value NaN, NaN is returned. </p><p> All items in $arg must be numeric or derived from a single base type. In addition, the type must support addition. Duration values must either all be xs:yearMonthDuration values or must all be xs:dayTimeDuration values. For numeric values, the numeric promotion rules defined in 6.2 Operators on Numeric Values are used to promote all values to a single common type. The sum of a sequence of integers will therefore be an integer, while the sum of a numeric sequence that includes at least one xs:double will be an xs:double. </p><p> If the above conditions are not met, a type error is raised [err:FORG0006]. </p><p> Otherwise, the result of the function, using the second signature, is the result of the expression: </p><pre> if (fn:count($c) eq 0) then $zero else if (fn:count($c) eq 1) then $c[1] else $c[1] + fn:sum(subsequence($c, 2)) </pre> <p> where $c is the converted sequence. </p><p> The result of the function, using the first signature, is the result of the expression:fn:sum($arg, 0). </p><p> For detailed type semantics, see <a>Section 7.2.10 The fn:min, fn:max, fn:avg, and fn:sum functions[FS]</a>. </p><p> Notes: </p><p> The second argument allows an appropriate value to be defined to represent the sum of an empty sequence. For example, when summing a sequence of durations it would be appropriate to return a zero-length duration of the appropriate type. This argument is necessary because a system that does dynamic typing cannot distinguish "an empty sequence of integers", for example, from "an empty sequence of durations". </p><p> If the converted sequence contains exactly one value then that value is returned. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.sum|fn.sum}
...
/**
    * <p> Returns the timezone component of $arg if any. If $arg has a timezone component, then the result is an xs:dayTimeDuration that indicates deviation from UTC; its value may range from +14:00 to -14:00 hours, both inclusive. Otherwise, the result is the empty sequence. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.timezoneFromDate|fn.timezoneFromDate}
...
/**
    * <p> Returns the items of $sourceSeq in an implementation dependent order. </p><p> Note: </p><p> Query optimizers may be able to do a better job if the order of the output sequence is not specified. For example, when retrieving prices from a purchase order, if an index exists on prices, it may be more efficient to return the prices in index order rather than in document order. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.unordered|fn.unordered}
...
/**
    * <p>Returns an xs:integer representing the year component in the localized value of $arg. The result may be negative. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.yearFromDate|fn.yearFromDate}
...
/**
    * <p>Returns an xs:integer representing the year component in the localized value of $arg. The result may be negative. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.yearFromDateTime|fn.yearFromDateTime}
...
/**
    * <p>Returns an xs:integer representing the years component in the canonical lexical representation of the value of $arg. The result may be negative. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/fn.yearsFromDuration|fn.yearsFromDuration}
...
/**
    * <p> Returns the sample covariance of a data set. The size of the input array should be 2. The function eliminates all pairs for which either the first element or the second element is empty. After the elimination, if the length of the input is less than 2, the function returns the empty sequence. </p> <p>For the version of this that uses range indexes, see <a>cts:covariance</a>. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/math.covariance|math.covariance}
...
/**
    * <p> Returns the population covariance of a data set. The size of the input array should be 2. The function eliminates all pairs for which either the first element or the second element is empty. After the elimination, if the length of the input is 0, the function returns the empty sequence. </p> <p>For the version of this that uses range indexes, see <a>cts:covariance-p</a>. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/math.covarianceP|math.covarianceP}
...
/**
    * <p>Returns a linear model that fits the given data set. The size of the input array should be 2, as currently only simple linear regression model is supported. The first element of the array should be the value of the dependent variable while the other element should be the value of the independent variable.</p> <p>The function eliminates all pairs for which either the first element or the second element is empty. After the elimination, if the length of the input is less than 2, the function returns the empty sequence. After the elimination, if the standard deviation of the independent variable is 0, the function returns a linear model with intercept = the mean of the dependent variable, coefficients = NaN and r-squared = NaN. After the elimination, if the standard deviation of the dependent variable is 0, the function returns a linear model with r-squared = NaN.</p> <p>For the version of this function that uses Range Indexes, see <a>cts:linear-model</a>.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/math.linearModel|math.linearModel}
...
/**
    * <p> Returns the mode of a sequence. The mode is the value that occurs most frequently in a data set. If no value occurs more than once in the data set, the function returns the empty sequence. If the input is the empty sequence, the function returns the empty sequence. </p><p> Note that a data set can have multiple “modes”. The order of multiple modes in the returned sequence is undefined. </p><p> Also note that values from a lexicon lookup are repeated <code>cts:frequency</code> times before calculating the mode. </p><p> The function can be used on numeric values, <code>xs:yearMonthDuration</code>, <code>xs:dayTimeDuration</code>, <code>xs:string</code>, <code>xs:anyURI</code>, <code>xs:date</code>, <code>xs:dateTime</code>, <code>xs:time</code>, and <code>cts:point</code>. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/math.mode|math.mode}
...
class RdfExpr {
  ...
  /**
    * Returns an <code>rdf:langString</code> value with the given value and language tag. The <code>rdf:langString</code> type extends <code>xs:string</code>, and represents a language tagged string in RDF. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/rdf.langString|rdf.langString}
...
/**
    * Returns the language of an <code>rdf:langString</code> value. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/rdf.langStringLanguage|rdf.langStringLanguage}
...
class SemExpr {
  ...
  /**
    * This function returns an identifier for a blank node, allowing the construction of a triple that refers to a blank node. This XQuery function backs up the SPARQL BNODE() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.bnode|sem.bnode}
...
/**
    * Returns the value of the first argument that evaluates without error. This XQuery function backs up the SPARQL COALESCE() functional form. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.coalesce|sem.coalesce}
...
/**
    * The IF function form evaluates the first argument, interprets it as a effective boolean value, then returns the value of expression2 if the EBV is true, otherwise it returns the value of expression3. Only one of expression2 and expression3 is evaluated. If evaluating the first argument raises an error, then an error is raised for the evaluation of the IF expression. This XQuery function backs up the SPARQL IF() functional form. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.if|sem.if}
...
/**
    * Returns the datatype IRI of a <code>sem:invalid</code> value. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.invalidDatatype|sem.invalidDatatype}
...
/**
    * Returns true if the argument is an RDF blank node - that is, derived from type <code>sem:blank</code>. This XQuery function backs up the SPARQL isBlank() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.isBlank|sem.isBlank}
...
/**
    * Returns true if the argument is an RDF IRI - that is, derived from type <code>sem:iri</code>, but not derived from type <code>sem:blank</code>. This XQuery function backs up the SPARQL isIRI() and isURI() functions. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.isIRI|sem.isIRI}
...
/**
    * Returns true if the argument is an RDF literal - that is, derived from type <code>xs:anyAtomicType</code>, but not derived from type <code>sem:iri</code>. This XQuery function backs up the SPARQL isLiteral() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.isLiteral|sem.isLiteral}
...
/**
    * Returns true if the argument is a valid numeric RDF literal. This XQuery function backs up the SPARQL isNumeric() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.isNumeric|sem.isNumeric}
...
/**
    * Returns the language of the value passed in, or the empty string if the value has no language. Only values derived from <code>rdf:langString</code> have a language. This XQuery function backs up the SPARQL lang() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.lang|sem.lang}
...
/**
    * Returns a random double between 0 and 1. This XQuery function backs up the SPARQL RAND() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.random|sem.random}
...
/**
    * Returns true if the arguments are the same RDF term as defined by the RDF concepts specification. This XQuery function backs up the SPARQL sameTerm() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.sameTerm|sem.sameTerm}
...
/**
    * The <code>sem:store</code> function defines a set of criteria, that when evaluated, selects a set of triples to be passed in to <code>sem:sparql()</code>, <code>sem:sparql-update()</code>, or <code>sem:sparql-values()</code> as part of the options argument. The <code>sem:store</code> constructor queries from the current database's triple index, restricted by the options and the <code>cts:query</code> argument (for instance, "triples in documents matching this query"). <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.store|sem.store}
...
/**
    * Returns the timezone of an <code>xs:dateTime</code> value as a string. This XQuery function backs up the SPARQL TZ() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.timezoneString|sem.timezoneString}
...
/**
    * Returns a <code>sem:unknown</code> value with the given literal value and datatype IRI. The <code>sem:unknown</code> type extends <code>xs:untypedAtomic</code>, and represents an RDF value with a datatype IRI for which no schema is available. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.unknown|sem.unknown}
...
/**
    * Returns the datatype IRI of a <code>sem:unknown</code> value. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.unknownDatatype|sem.unknownDatatype}
...
/**
    * Return a UUID URN (RFC4122) as a <code>sem:iri</code> value. This XQuery function backs up the SPARQL UUID() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.uuid|sem.uuid}
...
/**
    * Return a string that is the scheme specific part of random UUID URN (RFC4122). This XQuery function backs up the SPARQL STRUUID() function. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sem.uuidString|sem.uuidString}
...
/**
    * Returns an <code>rdf:collatedString</code> value with the given value and collation tag. The <code>rdf:collatedString</code> type extends <code>xs:string</code> , and represents a collation tagged string in RDF. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.collatedString|sql.collatedString}
...
/**
    * <p>Returns a specified date with the specified number interval (signed integer) added to a specified datepart of that date </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.dateadd|sql.dateadd}
...
/**
    * <p>Returns the count (signed integer) of the specified datepart boundaries crossed between the specified startdate and enddate. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.datediff|sql.datediff}
...
/**
    * <p>Returns an integer that represents the specified <code>datepart</code> of the specified <code>date</code>. </p><p> If <code>datepart</code> or <code>date</code> is the empty sequence, the function returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.datepart|sql.datepart}
...
/**
    * <p> Returns an xs:integer between 1 and 31, both inclusive, representing the day component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.day|sql.day}
...
/**
    * <p>Returns an xs:string representing the dayname value in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.dayname|sql.dayname}
...
/**
    * <p> Returns an xs:integer between 0 and 23, both inclusive, representing the value of the hours component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.hours|sql.hours}
...
/**
    * <p> Returns an xs:integer value between 0 to 59, both inclusive, representing the value of the minutes component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.minutes|sql.minutes}
...
/**
    * <p> Returns an xs:integer between 1 and 12, both inclusive, representing the month component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.month|sql.month}
...
/**
    * <p> Returns month name, calculated from the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.monthname|sql.monthname}
...
/**
    * <p> Returns an xs:integer between 1 and 4, both inclusive, calculating the quarter component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.quarter|sql.quarter}
...
/**
    * <p> Returns an xs:integer between 1 and 53, both inclusive, representing the week value in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.week|sql.week}
...
/**
    * <p> Returns an xs:integer representing the year component in the localized value of $arg. The result may be negative.</p> <p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.year|sql.year}
...
/**
    * <p>Returns an xs:integer between 1 and 366, both inclusive, representing the yearday value in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/sql.yearday|sql.yearday}
...
/**
    * <p>Returns an xs:string representing the dayname value in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/xdmp.daynameFromDate|xdmp.daynameFromDate}
...
/**
    * <p>Returns month name, calculated from the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/xdmp.monthNameFromDate|xdmp.monthNameFromDate}
...
/**
    * <p> Returns an <code>xs:string</code> representing the node's kind: either "document", "element", "attribute", "text", "namespace", "processing-instruction", "binary", or "comment". </p> <p> The <code>fn:node-kind</code> builtin was dropped from the final XQuery 1.0 spec. This is the equivalent function in the <code>xdmp:</code> namespace carried over for MarkLogic 1.0 dialects. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/xdmp.nodeKind|xdmp.nodeKind}
...
/**
    * <p>Returns an xs:integer between 1 and 4, both inclusive, calculating the quarter component in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/xdmp.quarterFromDate|xdmp.quarterFromDate}
...
/**
    * Returns the name of the simple type of the atomic value argument as an <code>xs:QName</code>. <p>This function is a built-in.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/xdmp.type|xdmp.type}
...
/**
    * <p>Converts plaintext into URL-encoded string. To decode the string, use <a>xdmp:url-decode</a>.</p> <p>There is also a W3C function that does a slightly different url encoding: <a>fn:encode-for-uri</a>.</p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/xdmp.urlEncode|xdmp.urlEncode}
...
/**
    * <p>Returns an xs:integer between 1 and 53, both inclusive, representing the week value in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/xdmp.weekFromDate|xdmp.weekFromDate}
...
/**
    * <p>Returns an xs:integer in the range 1 to 7, inclusive, representing the weekday value in the localized value of $arg. Monday is the first weekday value (value of 1), and Sunday is the last (value of 7). </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/xdmp.weekdayFromDate|xdmp.weekdayFromDate}
...
/**
    * <p>Returns an xs:integer between 1 and 366, both inclusive, representing the yearday value in the localized value of $arg. </p><p> If $arg is the empty sequence, returns the empty sequence. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/xdmp.yeardayFromDate|xdmp.yeardayFromDate}
...
/**
    * This method applies a function or the builtin reducer to each row returned by the plan to produce a single result as with the <code>reduce()</code> method of JavaScript Array. <p> The signature of the reducer must be <code>function(previous, row)</code>, where <code>previous</code> is the seed on the first request and the return from the previous call on subsequent request and <code>row</code> is the current row. </p> <p> The implementation of a <code>op:reduce</code> function can call <a>op:map</a> functions to chain map calls with reduce calls. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.reduce|op.reduce}
...
/**
    * This method collapses a group of rows into a single row. <p> If you want the results to include a column, specify the column either as a grouping key or as one of the aggregates. A group-by operation without a grouping key outputs a single group reflecting the entire row set. </p><p> The aggregates for a group by operation are specified as the second parameter instead of in a <a>op:select</a> operation (unlike SQL). </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.groupBy|op.groupBy}
...
this.xs = (fieldOverrides.xs !== void 0) ? fieldOverrides.xs : new XsExpr();
  ...
  /**
    * This function returns the sum of the specified numeric expressions. In expressions, the call should pass the result from an <a>op:col</a> function to identify a column. <p> The <a>op:add</a> function differs from the <a>op:sum</a> function in that it operates on operates on multiple column values in a row, rather than a group of rows. </p><p> As a convenience, you can pass a sequence of any number of expressions as the first argument: <code>op:add((expr1, expr2, ..., exprN))</code> </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.add|op.add}
...
/**
    * This function returns <code>true</code> if the specified expressions all return <code>true</code>. Otherwise, it returns <code>false</code>. You can either compair <p> As a convenience, you can pass a sequence of any number of expressions as the first argument: <code>op:and((expr1, expr2, ..., exprN))</code> </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.and|op.and}
...
/**
    * This function multipies the left <code>numericExpression</code> by the right <code>numericExpression</code> and returns the value. <p> As a convenience, you can pass a sequence of any number of expressions as the first argument: <code>op:multiply((expr1, expr2, ..., exprN))</code> </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.multiply|op.multiply}
...
/**
    * This function returns <code>true</code> if the specified expressions all return <code>true</code>. Otherwise, it returns <code>false</code>. <p> As a convenience, you can pass a sequence of any number of expressions as the first argument: <code>op:or((expr1, expr2, ..., exprN))</code> </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.or|op.or}
...
/**
    * <p> This function reads a row set from a configured view over TDE-indexed rows or a predefined view over range indexes. <p> This function creates a row set without a limit. Use <a>op:limit</a> or <a>op:offset-limit</a> to set a limit. </p> </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.fromView|op.fromView}
...
/**
    * This function adds the non-null values of the column for the rows in the group or row set. The result is used for building the parameters used by the <a>op:group-by</a> function. <p> The <a>op:sum</a> function differs from the <a>op:add</a> function in that it operates on operates on a group of rows, rather than multiple column values in a row. </p> Provides a client interface to a server function. See {@link http://docs.marklogic.com/op.sum|op.sum}
