...
/**
    ...
    */
    maskDeterministic(column, options) {
        ...
        const paramdefs = [['column', [PlanColumn, types.XsString], true, false], ['option', [], false, false]];
        const checkedArgs = (namer !== null) ?
            bldrbase.makeNamedArgs(namer, 'PlanBuilder.as', 2, new Set(['column', 'expression']), paramdefs, options) :
            bldrbase.makePositionalArgs('PlanBuilder.as', 2, false, paramdefs, options);
...
left(...args) {
    const namer = bldrbase.getNamer(args, 'str');
    const paramdefs = [['str', [types.Item, PlanColumn, PlanParam], false, true], ['n', [types.XsNumeric, PlanColumn, PlanParam], false, false]];
    const checkedArgs = (namer !== null) ?
...
/**
    * This function adds a column based on an expression without altering the existing columns in the row set. Provides a client interface to a server function. See {@link http://docs.marklogic.com/ModifyPlan.prototype.bindAs|ModifyPlan.prototype.bindAs}
