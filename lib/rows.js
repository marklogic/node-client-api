...
// reference whose first arg equals `name` with the given replacement node.
function substitutePlanParam(node, name, replacement) {
  if (node === null || typeof node !== 'object') { return node; }
  if (Array.isArray(node)) {
    return node.map(item => substitutePlanParam(item, name, replacement));
  }
  if ((node.ns === 'cts' || node.ns === 'op') && node.fn === 'param' &&
      Array.isArray(node.args) && node.args[0] === name) {
    return replacement;
  }
  const result = {};
  for (const key of Object.keys(node)) {
    result[key] = substitutePlanParam(node[key], name, replacement);
  }
  return result;
}

// For each entry in bindings, if the value is an exported plan-builder node
// ({ns, fn, args[]}), substitute it into planJson and collect the rest in
// remaining. Returns the (possibly updated) planJson and the remaining bindings.
function substituteExportedBindings(bindings, planJson, builtPlan) {
  const remaining = {};
  for (const [k, v] of Object.entries(bindings)) {
    ...
    if (exported && typeof exported === 'object' && typeof exported.ns === 'string' &&
      ...
      if (planJson === null) { planJson = builtPlan.export(); }
      planJson = substitutePlanParam(planJson, k, exported);
    } else {
      remaining[k] = v;
    }
  }
  return { planJson, remaining };
}
...
function queryRowsOperationImpl(self, builtPlan, streamType, options, bindingArg, graphqlQuery) {
  ...
  if (builtPlan instanceof planBuilder.Plan && (substitutedOptionBindings || bindingArg)) {
    ...
    if (substitutedOptionBindings) {
      let remaining;
      ({ planJson, remaining } = substituteExportedBindings(substitutedOptionBindings, planJson, builtPlan));
      if (planJson !== null) {
    ...
    if (bindingArg) {
      let remaining;
      ({ planJson, remaining } = substituteExportedBindings(bindingArg, planJson, builtPlan));
      if (planJson !== null) {
    ...
    if (planJson !== null) {
      builtPlan = JSON.stringify(planJson);
    }
  }
...
Rows.prototype.queryAsStream = function queryAsStreamRows() {
  const args = mlutil.asArray.apply(null, arguments);
  let streamType = 'chunked', // default
      options = null;
 ...
 */
 Rows.prototype.queryAll = function queryAllDocumentsImpl(batchView, jobOptions){
   ...
   const path = '/v1/internal/viewinfo';
...
Rows.prototype.execute = function executeRows() {
  ...
  const builtPlan = args[0], options = (args.length >= 2 && args[1])?args[1] : {};
  // opticExecute is an internal flag to ensure output doesn't default to anything.
...
function onQueryAllRows(jobState, readerId, batchNum) {
  ...
  const lowerBound = bigInt((currentBatchNum - 1) * jobState.requestBatchSize);
  const upperBound = (currentBatchNum === jobState.numberOfBatches) ? '-1' :
