...
function queryRowsOperationImpl(self, builtPlan, streamType, options, bindingArg, graphqlQuery) {
  ...
  switch (format) {
    case 'xml':
      ...
      responseType = 'single';
      endpoint += addOutputAndColumnTypes(options, structure,'column-types=' + columnTypes, sep);
      break;
    case 'csv':
      acceptHeader = 'text/csv';
      responseType = 'single';

      endpoint += addOutputAndColumnTypes(options, structure,'column-types=header', sep);
      if (sep === '?') { sep = '&'; }
      break;
    default:
...
function addOutputAndColumnTypes(options, structure, columnValue, sep) {
  if(options && options.update){
    const tempSep = (options.opticExecute)? '?':'&';
    return tempSep+columnValue;
  } else {
    return sep + 'output=' + structure + '&'+columnValue;
  }
}
