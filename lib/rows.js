...
function queryRowsImpl(builtPlan, streamType, options) {
  let sep = '?',
      ...
      validNodeColumns = ['inline','reference'],
  ...
  if (options.nodeColumns !== null && options.nodeColumns !== void 0) {
    if (validNodeColumns.indexOf(options.nodeColumns) >= 0) {
      nodeColumns = options.nodeColumns;
    } else {
      throw new Error('invalid node columns "' + options.nodeColumns + '"');
    }
  }
  ...
  switch (format) {
    case 'xml':
      if (streamType !== null && streamType === 'chunked') {
        // TODO use single document form for chunked streams, see spec
    ...
    default:
      ...
      if (sep === '?') { sep = '&'; }
      endpoint += (nodeColumns === 'reference') ? sep + 'node-columns=reference' : '';
      break;
  }
