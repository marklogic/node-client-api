...
function queryRowsOperationImpl(self, builtPlan, streamType, options, bindingArg, graphqlQuery) {
  ...
  let endpoint = createRowsEndpoint(options, graphqlQuery);
  if (options.bindings) {
  ...
  if (bindingArg) {
    ...
    const endpoint = createRowsEndpoint(options,graphqlQuery);
...
function createRowsEndpoint(options, graphqlQuery) {
  let endpoint = graphqlQuery?'/v1/rows/graphql':'/v1/rows';
  if(options && options.update){
    endpoint = options.structure?'/v1/rows/update?output='+options.structure:
        '/v1/rows/update';
  }
  return endpoint;
}
