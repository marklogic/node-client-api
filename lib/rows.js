...
function queryRowsOperationImpl(self, builtPlan, streamType, options, bindingArg, graphqlQuery) {
  ...
  if (builtPlan instanceof planBuilder.Plan && (substitutedOptionBindings || bindingArg)) {
    ...
    if (substitutedOptionBindings) {
      const remaining = {};
      for (const [k, v] of Object.entries(substitutedOptionBindings)) {
        if (v !== null && typeof v === 'object' && '_ns' in v && '_fn' in v && '_args' in v) {
          if (planJson === null) { planJson = builtPlan.export(); }
          planJson = substitutePlanParam(planJson, k, bldrbase.exportArg(v));
        } else {
          remaining[k] = v;
        }
      }
    ...
    if (bindingArg) {
      const remaining = {};
      for (const [k, v] of Object.entries(bindingArg)) {
        if (v !== null && typeof v === 'object' && '_ns' in v && '_fn' in v && '_args' in v) {
          if (planJson === null) { planJson = builtPlan.export(); }
          planJson = substitutePlanParam(planJson, k, bldrbase.exportArg(v));
        } else {
