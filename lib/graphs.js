...
function changeGraph(action, isStreaming, args) {
  ...
  var endpoint = null;
  switch(graphType) {
  case 'named':   endpoint = encodeURI('/v1/graphs?graph='+uri); sep = '&'; break;
...
Graphs.prototype.remove = function removeGraph() {
  ...
  return applyGraph.call(this, 'remove', uri, txid);
};
/**
...
Graphs.prototype.probe = function probeGraph() {
  ...
  return applyGraph.call(this, 'probe', uri, txid);
};
/** @ignore */
function applyGraph(action, uri, txid) {
  /*jshint validthis:true */
  var endpoint  = null;
  var graphType = null;
  var sep       = null;
  if (uri === null || uri === void 0) {
    ...
    graphType = 'named';
    endpoint  = encodeURI('/v1/graphs?graph='+uri);
    sep       = '&';
  }
...
Graphs.prototype.sparql = function queryGraphSPARQL() {
  ...
  var hasBegin = (begin !== null && begin >= 0);
  if (hasBegin) {
 ...
 * @param {boolean} [usingDefaultGraphs] - whether to use default graphs for the triples
 * in the update
 * @param {boolean} [usingNamedGraphs] - whether to use named graphs for the triples
 * in the update
 * @param {string} [base] - the base URI to prepend to relative resource identifiers
 * @param {string|transactions.Transaction}  [txid] - a string
 * transaction id or Transaction object identifying an open
...
Graphs.prototype.sparqlUpdate = function updateGraphSPARQL() {
  ...
  if (typeof usingDefaultGraphs === 'boolean') {
    endpoint += sep+'using-graph-uri='+String(usingDefaultGraphs);
    if (sep === '?') { sep = '&'; }
  } else if (usingDefaultGraphs instanceof Boolean) {
    endpoint += sep+'using-graph-uri='+usingDefaultGraphs.toString();
    if (sep === '?') { sep = '&'; }
  }

  if (typeof usingNamedGraphs === 'boolean') {
    endpoint += sep+'using-named-graph-uri='+String(usingNamedGraphs);
    if (sep === '?') { sep = '&'; }
  } else if (usingNamedGraphs instanceof Boolean) {
    endpoint += sep+'using-named-graph-uri='+usingNamedGraphs.toString();
    if (sep === '?') { sep = '&'; }
  }
...
function makeCommonSPARQLParams(
  ...
  if (rulesets !== null && rulesets !== void 0) {
    endpoint += (typeof rulesets === 'string' || rulesets instanceof String) ?
      (sep+'ruleset='+rulesets) :
      (sep+'ruleset='+rulesets.join('&ruleset='));
    if (sep === '?') { sep = '&'; }
  }
  if (defaultRulesets !== null && defaultRulesets !== void 0) {
    endpoint += sep+'defaultRulesets='+defaultRulesets;
    if (sep === '?') { sep = '&'; }
  }
  if (optimizeLevel !== null && optimizeLevel !== void 0) {
    endpoint += sep+'optimizeLevel='+optimizeLevel;
    if (sep === '?') { sep = '&'; }
  }
