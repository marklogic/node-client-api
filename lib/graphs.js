...
function readGraph() {
  ...
  requestOptions.path = valcheck.isNullOrUndefined(uri) ?
      '/v1/graphs?default' : encodeURI('/v1/graphs?graph='+uri);

// TODO: when streaming, should emit data chunks
...
function changeGraph(action, isStreaming, args) {
  ...
  if (argLen === 1 && !valcheck.isString(testArg)) {
    ...
    var maxArg = isStreaming ? argLen : argLen - 1;
    ...
    data = isStreaming ? null : args[maxArg];
  }
...
function queryGraphSPARQL() {
  ...
  if (hasNamedGraphs) {
    ...
    if (namedGraphs.length > 0) {
      ...
      // no subsequent URI parameters, so need to set sep
    } else {
