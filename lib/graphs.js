...
function identifyOutput(data, output) {
  ...
  const graphType = operation.graphType;
  switch(graphType) {
  case 'named':
...
function changeGraph(action, isStreaming, args) {
  ...
  if (argLen > 1 || (typeof args[0] === 'string' || args[0] instanceof String)) {
    ...
    data        = params.data;
    txid        = mlutil.convertTransaction(params.txid);
    const noContentType = (contentType == null);
    if (isStreaming) {
      if (noContentType) {
  ...
  const permissionsParams = makePermissionsParams(permissions, sep);
...
Graphs.prototype.list = function listGraphs() {
  ...
  const noContentType = (contentType === null || contentType === void 0);

  let endpoint = '/v1/graphs';
  let sep      = '?';
...
Graphs.prototype.sparql = function queryGraphSPARQL() {
  ...
  let defaultGraphs = null;
  let namedGraphs   = null;
  let acceptType    = null;
  let query         = null;
  ...
  let txid            = null;
  let rulesets        = null;
  let defaultRulesets = null;
  let optimizeLevel   = null;
  let bindings        = null;
  let timestamp       = null;
  ...
  let endpoint = '/v1/graphs/sparql';
  let sep = '?';

  let hasDefaultGraphs = (defaultGraphs !== null && defaultGraphs !== void 0);
  if (hasDefaultGraphs) {
  ...
  let hasNamedGraphs = (namedGraphs !== null && namedGraphs !== void 0);
  if (hasNamedGraphs) {
  ...
  const hasBegin = (begin !== null && begin !== void 0);
  if (hasBegin) {
    if (begin >= 0) {
  ...
  const commonParams = makeCommonSPARQLParams(
...
Graphs.prototype.sparqlUpdate = function updateGraphSPARQL() {
  ...
  let permissions        = null;
  let usingDefaultGraphs = null;
  let usingNamedGraphs   = null;
  let base               = null;
  let txid               = null;
  let rulesets           = null;
  let defaultRulesets    = null;
  ...
  const arg  = args[0];
  let data = (argLen === 1) ? arg.data : null;
  ...
  let hasUsingDefaultGraphs = (usingDefaultGraphs !== null && usingDefaultGraphs !== void 0);
  ...
  let hasUsingNamedGraphs = (usingNamedGraphs !== null && usingNamedGraphs !== void 0);
