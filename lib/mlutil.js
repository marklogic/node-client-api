...
const multipartBoundary = 'MLBOUND_' + Date.UTC(2014,12,31);
...
function copyProperties(source, target, srcKeys) {
  const dest    = (arguments.length > 1) ? target  : {};
  const isNamed = Array.isArray(srcKeys);
  const keys    = isNamed ? srcKeys : Object.keys(source);
    ...
    if (Array.isArray(transform)) {
      ...
      default:
        ...
        for (let i=0; i < transformKeys.length; i++) {
          const transformKey = transformKeys[i];
...
function newRequestOptions(connectionParams, endpoint, method) {
  ...
  if(connectionParams.basePath) {
    const basePath = connectionParams.basePath.toString();
    let fixedBasePath = (connectionParams.host.toString().endsWith('/') && basePath.startsWith('/')) ? basePath.substring(1):basePath;
    fixedBasePath = fixedBasePath.endsWith('/') ? fixedBasePath.substring(0,fixedBasePath.length-1):fixedBasePath;
    endpoint = fixedBasePath+endpoint;
  }
...
function makeSliceClause(variant, args) {
  ...
  const argMax = Math.min(argLen, ((variant === 'query') ? 5 : 3));
  let arg    = null;
  ...
  if (firstArg !== null && secondArg !== 0) {
    const pageStart = (sliceMode === 'legacy') ? firstArg : firstArg + 1;
