...
/**
 * Specifies a role to add to a document's permissions.
 * @method
 * @memberof! patchBuilderPermissions
 * @param {object} permission - an object with one role and an array of
 * one or more capabilities from the insert|update|read|execute enumeration
 * @returns {patchBuilder.PatchOperation} a patch operation
 */
function addPermission(permission) {
  if (permission == null || permission['role-name'] == null ||
      permission.capabilities == null) {
    throw new Error('permissions.add() takes a permission object with role-name and capabilities properties');
  }
 ...
 * @method
 * @memberof! patchBuilderPermissions
 * @param {object} permission - an object with the existing role and an array
 * with the role's modified capabilities from the insert|update|read|execute enumeration
 * @returns {patchBuilder.PatchOperation} a patch operation
 */
function replacePermission(permission) {
  if (permission == null || permission['role-name'] == null ||
      permission.capabilities == null) {
    throw new Error('permissions.replace() takes a permission object with role-name and capabilities properties');
  }
  return replace(
