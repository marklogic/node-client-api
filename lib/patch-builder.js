...
function library(module) {
  ...
  const rootname = mlutil.rootname(module);
  if (rootname === null) {
    throw new Error('library must have an extension of .sjs or .xqy');
  }
  const extension = module.substring((module.lastIndexOf('.')+1), module.length);
...
function pathLanguage() {
  const pathlang = (arguments.length < 1) ? null : arguments[0];
...
function getPermission(args) {
  ...
  const first = (argLen === 0) ? null : args[0];
  if (first !== null) {
  ...
  const check = {execute:true, insert:true, read:true, update:true};
  if (Array.isArray(capabilities)) {
    const max = capabilities.length;
    for (let i=0; i < max; i++) {
