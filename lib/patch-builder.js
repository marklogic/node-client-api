...
function library(module) {
  ...
  return {'replace-library':{
    ns: '/ext/marklogic/patch/apply/'+rootname,
 ...
 * @param ...args - arguments to pass to the applied function
...
function apply() {
  ...
  var applyCall = {apply: arguments[0]};
  if (argLen > 1) {
    var content = [];
    for (var i=1; i < argLen; i++) {
      content.push({$value: arguments[i]});
    }
    applyCall.content = content;
  }
