...
function parsePartHeaders(headers) {
  ...
  // TODO: versionId parameter of content disposition
...
function initClient(client, connectionParams) {
  ...
  if (isSSL) {
    ...
    if (noAgent) {
      ['ca', 'cert', 'ciphers', 'key', 'pfx', 'rejectUnauthorized'].
      ...
      connectionParams.agent = new KeepAlive.HttpsAgent(agentOptions);
    }
  } else {
    client.request = http.request;
    if (noAgent) {
      // may need to be configurable; seems to delay termination for HTTPS
      agentOptions.keepAliveMsecs = 6000;
      connectionParams.agent = new KeepAlive(agentOptions);
    }
  }
}  
    ...
    if (statusCode >= 400) {
      response.pipe(concatStream(
          ...
          function(body) {
            ...
            operation.dispatchEndError(errMsg+'/n'+body);
          }));
    } else {
    ...
    if (hasContentType) {
      if (contentType.match(/^multipart\/mixed(;.*)?$/)) {
        ...
        if (hasFormat) {
          ...
          format = contentType.replace(/^(text)\/.*$/, '$1');
          hasFormat = !valcheck.isNullOrUndefined(format);
          if (hasFormat) {
            isString = true;
          }
