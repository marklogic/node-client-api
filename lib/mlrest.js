...
function multipartResponder(response) {
  ...
  if (operation.outputStreamMode === 'chunked') {
    operation.initPartChunkedParser(parser);
  } else {
    operation.initPartObjectParser(parser);
  }
...
function initPartChunkedParser(parser) {
  // TODO: set chunk writer in operation during configuration
  // create paused stream before the initial request returns
  // resume for stream or promise
  // for promise, concat chunked with data listener
  var chunkWriter = through2();

  parser.on('part', function partListener(partReadStream) {
    partStream.pipe(chunkWriter, {end: false});
    partStream.on('error', errorListener);
  });

  parser.on('finish', function partFinishListener() {
    chunkWriter.end();
    chunkWriter = null;
  });
...
Operation.prototype.initPartChunkedParser = initPartChunkedParser;

function initPartObjectParser(parser) {
/* Note about Dicer:
+ each type of listener starts each part in order 
+ different types of listeners interleave
+ finish follows contract
part listener 1
header listener 1
part listener 2
header listener 2
part data listener 1
part data listener 2
part end listener 1
part end listener 2
finish listener
partsQueue an array with a PartBuffer per part - rawHeaders, headers, stream, data
nextPart an integer
on finish, set partsDone, call flushParts, and remove drain listener
pipe readable stream for part to concat
in concat callback, set stream to null, set data, and call pipeParts
concat callback must have access to its buffer
pipeParts
    call writeParts
    if writeParts returns false, pause all active part readers and set isPaused to true
writeParts
    if nextPart is metadata and not isInline and not isDone, stop
    after writing part, advance nextPart
    if write returns false, return false
drainParts
    drain event listener for through2
    call writeParts
    if writeParts returns true, resume all active part readers and set isPaused to true
either pause each part stream until ready or queue 
 */
  var operation = this;

  // TODO: set object writer in operation during configuration
  // create paused stream before the initial request returns
  // resume for stream or promise
  // for promise, accumulate array with data listener
  var objectWriter = through2({objectMode: true});

  objectWriter.on('drain', function partDrainListener() {
    // TODO: drain and resume
  });

  var headerBuffer = [];
  var nextHeader = 0;
  var partHeadersListen = function partHeadersListener(headers) {
    var partBuffer = partsQueue[nextHeader];
    nextHeader++;

    setPartHeaders(operation, partBuffer, headers);
  };

  var partReaderBuffer = [];
  var currentPartReader = 0;

  var objectBuffer = new Array(2);
  var firstObject = 0;
  var lastObject  = -1;
  var parseFinished = false;
  var drainObjectBuf = function drainObjectBuffer() {
...
// TODO: allow for empty data
  var partConcat = function partConcatenator(data) {
    var isBuffer = valcheck.isBuffer(data);

    var partData = null;
    if (isJSON && isBuffer) {
      partData = JSON.parse(data.toString());
    } else if (isJSON) {
      partData = JSON.parse(data);
    } else if (isUtf8 && isBuffer) {
      partData = data.toString();
    } else {
      partData = data;
    }

    // TODO: service-specific data changes using rawHeaders before writing to pipeline
// TODO: queue object
    var partObject = null;
    if (isInline) {
      operation.logger.debug('finished parse of inline part');
      if (metadataBuffer !== null) {
        operation.logger.debug('dispatching standalone metadata part for %s', metadataBuffer.uri);
        mlutil.copyProperties(metadataBuffer, metadataBuffer.content);
        writableMetadata = metadataBuffer.content;
        metadataBuffer = null;
      }
      operation.logger.debug('dispatching inline part');
      // TODO: identify the kind of inline part
      writableData = partData;
    } else if (isMetadata) {
      operation.logger.debug('finished parse of %s metadata', partUri);
      if (metadataBuffer !== null) {
        operation.logger.debug('dispatching standalone metadata part for %s', metadataBuffer.uri);
        mlutil.copyProperties(metadataBuffer, metadataBuffer.content);
        writableMetadata = metadataBuffer.content;
      ...
      headersBuffer.content = partData;
      metadataBuffer = headersBuffer;
    } else {
      operation.logger.debug('finished parse of %s content', partUri);
      if (metadataBuffer !== null) {
        if (metadataBuffer.uri !== partUri) {
          operation.logger.debug('dispatching standalone metadata part for %s', metadataBuffer.uri);
          mlutil.copyProperties(metadataBuffer, metadataBuffer.content);
          writableMetadata = metadataBuffer.content;
          metadataBuffer = null;
          ...
          mlutil.copyProperties(metadataBuffer.content, headersBuffer);
          metadataBuffer = null;
        }
      }
      operation.logger.debug('dispatching content part for %s', partUri);
      headersBuffer.content = partData;
      writableData = headersBuffer;
    }
    headersBuffer = null;

    partReaderBuffer[currentPartReader] = null;
    currentPartReader++;
...
// TODO: drain
  ...
  parser.on('part', function partListener(partReadStream) {
    partReaderBuffer.push(partReadStream);
    partReadStream.on('header', partHeadersListen);
    partReadStream.pipe(partConcat, {end: false});
...
Operation.prototype.initPartObjectParser = initPartObjectParser;

function PartBuffer(partStream) {
  if (!(this instanceof PartBuffer))
    return new PartBuffer(partStream);
...
function setPartHeaders(operation, partBuffer, headers) {
  partBuffer.rawHeaders = headers;

  var partHeaders    = parsePartHeaders(headers);
  partBuffer.headers = partHeaders;

  var isInline = (partHeaders.partType === 'inline');
  if (isInline) {
    partBuffer.isInline = isInline;
  }
  ...
  if (isMetadata) {
    partBuffer.isMetadata = isMetadata;
  }
  ...
  if (valcheck.isNullOrUndefed(format) && (isInline || isMetadata)) {
    format             = 'json';
    partHeaders.format = format;
  }
  ...
  if (isUtf8) {
    partBuffer.stream.setEncoding('utf8');
    partBuffer.isUtf8 = isUtf8;
  }
};
function receivePartData(operation, partBuffer, data) {
  var partHeaders = partBuffer.headers;

  var isInline = partBuffer.isInline;
  var isUtf8   = partBuffer.isUtf8;

  var isBuffer = valcheck.isBuffer(data);
  var isJSON   = (partHeaders.format === 'json');

  var partData = null;
  if (valcheck.isNullOrUndefined(data) || data.length === 0) {
    partData = isBuffer ? data : '';
  } else if (isJSON && isBuffer) {
    partData = JSON.parse(data.toString());
  } else if (isJSON) {
    partData = JSON.parse(data);
  } else if (isUtf8 && isBuffer) {
  ...
  // TODO: service-specific data changes using rawHeaders
  partBuffer.data = partData;

  if (isInline) {
    // TODO: identify the kind of inline part
    operation.logger.debug('finished parse of inline part');
  } else if (partBuffer.isMetadata) {
  ...
  partBuffer.stream = null;
};

// TODO: DELETE START
function initPartParserOLD(parser) {
  var operation = this;
