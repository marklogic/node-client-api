...
/* TODO: better stream spy for debug logging
  var debugLevel = winston.levels['debug'];
  var logLevel   = winston.levels[operation.logger.transports.Console.level];
  if (logLevel >= debugLevel) {
  if (false) {
    multipartStream.pipe(through2(function multipartSpy(chunk, encoding, done) {
...
MultipartDispatcher.prototype.promise = function dispatchMultipartPromise(
  ...
  var finishObjects = function promisePartFinishObjects() {
    if (parsedParts === partReaderQueue.getTotal()) {
  ...
  var queueObject = function promisePartQueueObject(data) {
    ...
    if (partReaderQueue.hasItem()) {
      ...
      operation.logger.debug('piped part %d', (parsedParts + 1));
      partReadStream.pipe(concatStream(queueObject));
    } else if (queuedAll) {
      finishObjects();
    }
  };
  ...
  var partListener = function promisePartListener(partReadStream) {
    ...
    if (partReaderQueue.isLast()) {
      operation.logger.debug('queued part %d with pipe', partReaderQueue.getTotal());
      partReadStream.pipe(concatStream(queueObject));
    } else {
      operation.logger.debug('queued part %d without pipe', partReaderQueue.getTotal());
    }
  };

  var partFinishListener = function promisePartFinishListener() {
    operation.logger.debug('queued all parts');
    queuedAll = true;
    if (!partReaderQueue.hasItem()) {
      finishObjects();
    }
  ...
  parser.on('finish', partFinishListener);
...
MultipartDispatcher.prototype.chunkedStream = function dispatchMultipartChunkedStream(
  ...
  var partFinishListener = function chunkedPartFinishListener() {
    outputStream.end();
    outputStream       = null;
    partFinishListener = null;
    partListener       = null;
...
MultipartDispatcher.prototype.objectStream = function dispatchMultipartObjectStream(
  ...
  var partFinishListener = function objectPartFinishListener() {
