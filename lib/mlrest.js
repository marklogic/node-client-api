...
MultipartDispatcher.prototype.promise = function dispatchMultipartPromise(
  ...
  var partFinisher = function promisePartFinisher(data) {
    ...
    if (parsedParts === parsingParts) {
      if (queuePhase === 'queuedAll') {
        responseFinisher();
      } else if (queuePhase === 'queueing') {
        parser.emit('drain');
      }
    }
  };
  ...
  var queueFinisher = function promiseQueueFinisher() {
    if (queuePhase === 'queueing') {
      operation.logger.debug('ended all parts');
      queuePhase = 'queuedAll';
    }
    if (parsedParts === parsingParts) {
      responseFinisher();
    }
...
MultipartDispatcher.prototype.objectStream = function dispatchMultipartObjectStream(
  ...
  var partTransform = function objectPartQueueTransform(
    ...
    } else if (parsedParts === parsingParts) {
      parser.emit('drain');
    }
  };
