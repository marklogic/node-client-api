...
MultipartDispatcher.prototype.promise = function dispatchMultipartPromise(
  ...
  var partFinisher = function promisePartFinisher(data) {
    ...
    if (partReaderQueue.hasItem()) {
      ...
      operation.dispatchError('received part after dispatching promise');
    } else if (hasParsed) {
      responseFinisher();
    } else if (!hasEnded) {
      parser.emit('drain');
  ...
  var parseFinisher = function promiseParseFinisher() {
  ...
  var responseEndListener = function promiseResponseEndListener() {
    ...
    if (!hasParsed && !partReaderQueue.hasItem()) {
      operation.logger.debug('response ended at part %d of %d', parsedParts, parsingParts);
      parseFinisher();
    }
  };
  ...
  // TODO: remove temporary workaround for finish bug
...
MultipartDispatcher.prototype.chunkedStream = function dispatchMultipartChunkedStream(
  ...
  var partEndListener = function chunkedPartEndListener() {
    ...
    if (partReaderQueue.hasItem()) {
      ...
      operation.dispatchError('received part after dispatching chunks');
    } else if (hasParsed) {
      responseFinisher();
    } else if (!hasEnded) {
      parser.emit('drain');
  ...
  var parseFinisher = function chunkedParseFinisher() {
    if (hasResolved) return;
  ...
  var responseEndListener = function chunkedResponseEndListener() {
    ...
    hasEnded = true;
    if (!hasParsed && !partReaderQueue.hasItem()) {
      operation.logger.debug('response ended');
      parseFinisher();
    }
  };
...
MultipartDispatcher.prototype.objectStream = function dispatchMultipartObjectStream(
  ...
  var responseEndListener = function promiseResponseEndListener() {
    ...
    hasEnded = true;
    if (!hasParsed && queuedReader !== null) {
      queuedReader.queuedAll();
    }
  };
