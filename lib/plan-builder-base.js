...
function castArg(arg, funcName, paramName, argPos, paramTypes) {
  if (arg === void 0 || arg === null || paramTypes === void 0 || paramTypes === null ||
        paramTypes.some(paramType => (arg instanceof paramType))) {
    return arg;
  } else if (arg instanceof types.ServerType) {
    throw new Error(
      `${argLabel(funcName, paramName, argPos)} must have type ${typeLabel(paramTypes)}`
    );
/* TODO: DELETE OR REFACTOR Number etc
...
*/
  ...
  switch(argType) {
// TODO: only where types.XsBoolean?
  case 'boolean':
// TODO: only where types.XsNumeric?
  case 'number':
  case 'string':
    return arg;
  default:
    const paramType = (paramTypes.length === 1) ? paramTypes[0] : null;
    if (Array.isArray(paramTypes) && paramTypes.some(paramType => {
