...
// TODO: throw new error if client is null
  ...
  const classMethods = hasSession ? [constructor, sessionFactory] : [constructor];
...
// TODO: client-side data type including SessionState
  return `
    ...
    transform(directory, encoding, callback) {
      ...
      readDeclarationsFromDirectory(directory.path)
          .then(result => {
            ...
            const proxySrc = generateModuleSource(serviceDeclaration, result.functionDeclarations);
            const moduleName = serviceDeclaration.$jsModule;
            const cwd = directory.cwd;
            const outputBase = path.resolve(cwd, 'lib');
            const outputRelative =
                (moduleName === void 0 || moduleName === null || moduleName.length === 0) ?
                directory.basename + '.js' : moduleName;
