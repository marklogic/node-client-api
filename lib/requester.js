...
function startRequest(operation) {
  ...
  operation.errorListener = operationErrorListener.bind(operation);
  ...
  case 'chunked':
    ...
    started = through2();
    started.result = operationResultPromise.bind(operation);
  ...
  var needsAuthenticator = (
    options.authType.toUpperCase() === 'DIGEST' ||
    (options.authType.toUpperCase() === 'KERBEROS')
  );
  var authenticator = null;
  if (needsAuthenticator) {
    if (options.authType.toUpperCase() === 'DIGEST') {
      authenticator = getAuthenticator(operation.client, options.user);
    } else if (options.authType.toUpperCase() === 'KERBEROS') {
...
function authenticatedRequest(operation, authenticator, requester) {
  ...
  var request = operation.client.request(
      options, responseDispatcher.bind(operation)
      );
