...
function getAccessToken(operation, callback){
  ...
  const req = https.request(options, (res) => {
    res.on('data', (d) => {
      operation.accessToken = JSON.parse(d.toString()).access_token;
      callback(operation);
    });
    res.on('error', (e) => {
...
function startRequest(operation) {
  ...
  if (needsAuthenticator) {
    ...
    if(operation.client.connectionParams.apiKey && !operation.accessToken){
      getAccessToken(operation,authenticatedRequest);
    } else {
...
function multipartRequester(request) {
  ...
  if(operation.bindingParam) {
    ...
    const query = operation.bindingParam.query;
    const key = operation.bindingParam.key;
    const binding = operation.bindingParam[key];
