...
/**
 * An object with a {@link ResultProvider#result} function for receiving
 * the database result. 
 ...
 * or a {@link ResultStreamProvider#stream} function for receiving the results
 * from the database request. 
 * @namespace ResultStreamProvider
 * @augments ResultProvider
 */
/**
 * Returns a ReadableStream object in object mode for receiving results as
 * complete objects.
 * @name ResultStreamProvider#stream
 * @function
 ...
 * @param {object} document - a sparse document descriptor with an exists
 * property that identifies whether the document exists 
 */

/**
 * Specifies checks whether a document exists; takes a configuration
...
function patchDocuments() {
  ...
  if (params !== null) {
    ...
    if (valcheck.isArray(arguments[1])) {
      ...
      documentOperations = Array.prototype.slice.call(arguments, 1);
    }
  }
