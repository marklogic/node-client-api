 ...
 * @param {boolean} [consistentSnapshot] - Uses the timestamp of the first request for all subsequent requests to yield
 *  an immutable view of the result set.
...
function queryAllDocumentsImpl(query, jobOptions) {
  ...
  if(jobState.jobOptions.queryBatchMultiple && (jobState.jobOptions.queryBatchMultiple <=0 ||
      jobState.jobOptions.queryBatchMultiple>jobState.jobOptions.batchSize)){
    jobState.jobOptions.queryBatchMultiple = null;
  }
...
function onDocUrisCollected(output) {
  ...
  if(output) {
    ...
    if(jobState.jobOptions.queryBatchMultiple){
      ...
      while(i<uriArr.length){
        ...
        (uriArr.slice(i, writeBatchSize)).forEach(uri => jobState.stream.write(uri));
        i = writeBatchSize;
      }
    }
    else {
