...
function writeAllDocumentsImpl(options) {
  var path = '/v1/internal/forestinfo';
  var connectionParams = this.client.getConnectionParams();
  var requestOptions = mlutil.copyProperties(connectionParams);
  ...
  requestOptions.stream = fileStream;
  requestOptions.path = mlutil.databaseParam(connectionParams, path);
  var operation = new Operation(
      'read forestInfo', this.client, requestOptions, 'empty', 'single'
  );
  var result = requester.startRequest(operation);
  console.log(result);
  return fileStream;
//add a request to call

  // create an operation to hit endpoint - /v1/internal/forestinfo
  // create a passthrough stream
  // add the passthrough in the operation
  // start a request to get the forest information (DataMovementManagerImpl com.marklogic.client.datamovement.impl.DataMovementServices#readForestConfig) but it
  // requester.startRequest(operation).result(onSuccess);
