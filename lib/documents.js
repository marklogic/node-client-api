 ...
 * @returns {stream.Passthrough} - a stream.Passthrough in object mode that sends document URI output read from the database
 ...
 *  Throws error if consistentSnapshot is not true.
 * @param {chunked|object} [outputStreamType] - Controls whether data is written to the output stream as chunks or object.
 * @returns {stream.Passthrough} - a stream.Passthrough in object mode that returns document descriptors with the content
...
function transformAllDocumentsImpl(jobOptions){
  ...
  if(jobState.jobOptions.transformStrategy){
    if (!new Set(['replace', 'ignore']).has(String(jobState.jobOptions.transformStrategy))) {
...
function onTransformAllInit(output) {
  const jobState = this;
  let endpoint = '/v1/internal/apply-transform?start=1&pageLength='+jobState.jobOptions.batchSize;

  if(jobState.jobOptions.transformStrategy){
    endpoint = endpoint+'&result='+String(jobState.jobOptions.transformStrategy);
  }
  ...
  let operation = new Operation(
      'transform docs', jobState.docInstance.client, requestOptions, 'multipart', 'single'
  );
  operation.validStatusCodes  = [200, 204, 404];

  jobState.endpoint = endpoint;
  jobState.operation = operation;
...
function onTransformAllDocs(jobState, writerId) {
  ...
  if(jobState.inputKindValue  && jobState.inputKindValue === 'array'){
  ...
  if (transformBatchArray.length !== 0) {
    console.log(transformBatchArray.length)
    const batchdef = {documents: transformBatchArray};
    if(jobState.jobOptions.transform){
      batchdef.transform = jobState.jobOptions.transform;
    }
    transformDocs(jobState, [batchdef],transformBatchArray, writerId);
  }
}

function transformDocs(jobState, batchdef,transformBatchArray, writerId){

  jobState.operation.documents = batchdef;

  requester.startRequest(jobState.operation)
      .result((output) => {
      ...
      .catch((err) => {
        ...
        switch(jobState.retryCount){
          ...
          case 1: {
            jobState.retryCount--;
            batchdef.documents = errorDisposition;
            transformDocs(jobState, [batchdef],batchdef.documents, writerId);
            break;
          }
          default: {
            jobState.docsFailedToBeTransformed += transformBatchArray.length;
