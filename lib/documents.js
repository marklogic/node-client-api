...
function queryAllDocumentsImpl(query, jobOptions) {
  ...
  if(jobState.jobOptions.batchSize){
    if(jobState.jobOptions.batchSize > 100000) {
      throw new Error('BatchSize cannot be greater than 100000');
    }
  } else if(!jobState.jobOptions.batchSize || jobState.jobOptions.batchSize<0){
    jobState.jobOptions.batchSize = 1000;
  }

  jobState.stream = (jobState.jobOptions.batchSize === 1)? new stream.PassThrough():
      new stream.PassThrough({objectMode: true});

  jobState.jobOptions.queryBatchMultiple = (jobState.jobOptions.queryBatchMultiple &&
      jobState.jobOptions.queryBatchMultiple<jobState.jobOptions.batchSize) ? jobState.jobOptions.queryBatchMultiple:null;
...
function onQueryUrisRequest(output) {
  const jobState = this;
  let endpoint = '/v1/internal/uris?filtered=false&start=1&pageLength='+jobState.jobOptions.batchSize;
  ...
  if(jobState.jobOptions.consistentSnapshot){
    jobState.forestDict[jobState.forests[0].name] = null;
...
function onDocUrisCollected(output) {
  ...
  if(jobState.jobOptions.consistentSnapshot && !jobState.serverTimestamp){
    jobState.serverTimestamp = output.timeStamp;
    spinForestRequests(jobState,1);
  }
  if(output) {
    ...
    if(jobState.queryBatchMultiple){
      const writeSize = jobState.jobOptions.batchSize/jobState.jobOptions.queryBatchMultiple;
      let i=0;
...
function collectDocUris(forestName, jobState){
  ...
  if(jobState.serverTimestamp){
    forestEndpoint = forestEndpoint+'&timestamp='+jobState.serverTimestamp;
  }
      ...
      .catch((err) => {
        ...
        if (jobState.jobOptions.onBatchError) {
          const progressSoFar = {
            ...
            urisFailedToBeRead: jobState.jobOptions.batchSize,
            timeElapsed: (Date.now()-jobState.startTime),
            forestName: forestName,
            lastUriReadSuccessfully: jobState.forestDict[forestName].AfterUri,
            retryCount: jobState.retryCount
          };
          const error = errorDisposition;
          try {
            errorDisposition = jobState.jobOptions.onBatchError(progressSoFar, error);
          } catch(err){
            errorDisposition = err;
          }
        }
        ...
        if(errorDisposition instanceof Error){
          ...
          if(jobState.retryCount >100){
            jobState.stream.emit('error', new Error('RetryCount exceeded for ths batch.'));
            jobState.requesterCount = 0;
            finishOnQueryAllDocs(jobState);
          }
          jobState.retryCount+=1;
          onDocUrisCollected(forestName, jobState);
        } else if(!errorDisposition){
          jobState.forestDict[forestName].Error = err;
          jobState.requesterCount-= 1;
          return;
        } else {
          jobState.stream.emit('error', new Error('onBatchError should send true, false or an Error'));
          jobState.requesterCount = 0;
          finishOnQueryAllDocs(jobState);
        }
      });
...
function finishOnQueryAllDocs(jobState){
  ...
  if(jobState.requesterCount <= 0) {
    ...
    if(jobState.jobOptions.onCompletion){
      ...
      for (const forest in jobState.forestDict) {
        if(jobState.forestDict[forest].Error){
          summary.batchFailureError = {
            ErrorOccurredOnForest: forest,
            ErrorInfo: jobState.forestDict[forest].Error,
            LastUriReadFromThisForest: jobState.forestDict[forest].AfterUri
          };
        }
      }
      if (jobState.error) {
...
function urisOutputTransform(headers, data) {
  ...
  const timeStamp = headers['ML-Effective-Timestamp'];
  const urisWithTimestamp = {uriArr: uriArr.slice(0, uriArr.length-1), timeStamp:timeStamp};
  return urisWithTimestamp;
}
