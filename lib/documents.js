...
// TODO: transform, range[from, to]
  ...
  var path = '/v1/documents?format=json&uri='+uri;
  path += '&category=' + (
...
function createWriteStream(document) {
  // TODO: verify the document has a uri and no data

  var responseProxy = mlrest.response(resultEventMapper, true, document);
...
function writeDocuments() {
  ...
  var responseProxy = mlrest.response(resultEventMapper, true, null);
...
function removeAllDocuments(params) {
  ...
  var responseProxy = mlrest.response(mlrest.emptyEventMapper, true, params);
