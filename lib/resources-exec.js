...
function makeRequestOptions(client, args) {
  let path = '/v1/resources/'+args.name;

  let sep ='?';

  const params = args.params;
  const keys = (params === void 0) ? null : Object.keys(params);
  const keyLen = (keys === null) ? 0 : keys.length;
  let key = null;
  let prefix = null;
  let i=0;
  let value = null;
  ...
  const txid = mlutil.convertTransaction(args.txid);
...
function addPart(operation, document) {
  ...
  const contentType   = hasContent ? document.contentType : null;

  if (hasContent && (contentType != null)) {
    const marshaledData = mlutil.marshal(content, operation);
