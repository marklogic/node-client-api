...
function before() {
   ...
   if(!(typeof(args[0]) == 'string')) {
     if(typeof(args[0]) == 'number') {
      ...
      var epochMilli = args[0].getTime();
      arg = new String(epochMilli);
     }
     else {
      throw new Error('The timestamp argument needs to be either String, Number or Date');
     }
   }
  return new BeforeQueryDef(new TimestampDef(arg));
}
/** @ignore */
...
function after() {
  ...
  return new AfterQueryDef(new TimestampDef(arg));	
}
/** @ignore */
