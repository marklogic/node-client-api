...
function slice() {
  ...
  for (var i=0; i < argMax; i++) {
    ...
    } else if (!valcheck.isUndefined(arg['extract-result-documents'])) {
      sliceClause['extract-result-documents'] = arg['extract-result-documents'];
    }
  }
...
function extract() {
  ...
  return {'extract-result-documents': extractdef};
}
 ...
 * @param {Array}  [transform] - the name and, optionally, parameters for a transform to apply
 * to results documents; the transform must have been installed on server previously
