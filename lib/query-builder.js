...
function currentTime() {
  ...
  var argLen = args.length;
  if (argLen < 1) {
    throw new Error('no temporal collection for current time query');
  }
  ...
  for (var i=0; i < args.length; i++) {
    ...
    if (i === 0) {
      ...
      query['query-option'] = arg['temporal-option'];
...
function period() {
  ...
  if (valcheck.isNull(startDate) || valcheck.isNull(endDate)) {
    throw new Error('period must have start and end date');
  }

  return {period: {'period-start': startDate, 'period-end': endDate}};
}
function periodCompare() {
  ...
  if (!valcheck.isNull(temporalOption)) {
    query['query-option'] = temporalOption;
  }
...
function periodRange() {
  ...
  var arg = (argLen > 2) ? arguments[0] : null;
  var rangeAxis = valcheck.isString(arg) ? arg : null;

  arg = (argLen > 2) ? arguments[1] : null;
  var rangeOperator = valcheck.isString(arg) ? arg : null;

  arg = (argLen > 2) ? arguments[2] : null;
  arg = (!valcheck.isNullOrUndefined(arg)) ? arg.period : null;
  var rangePeriod = (!valcheck.isNullOrUndefined(arg)) ? arg : null;

  arg = (argLen > 3) ? arguments[3] : null;
  arg = (!valcheck.isNullOrUndefined(arg)) ? arg['temporal-option'] : null;
  var temporalOption = (!valcheck.isNullOrUndefined(arg)) ? arg : null;

  if (valcheck.isNull(rangeAxis) || valcheck.isNull(rangeOperator) ||
      valcheck.isNull(rangePeriod)) {
