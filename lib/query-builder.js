...
function field() {
  ...
  case 1:
    return {field: {name: args[0]}};
  default:
    return {field: {name: args[0], collation: args[1]}};
 }
}
/**
...
function geoAttributePair() {
  ...
  for (var i=0; i < keys.length; i++) {
    ...
    var arg = args[iArg++];
    if (arg.qname) {
      location[key] = arg.qname;
    } else if (valcheck.isString(arg)) {
      location[key] = nsName.call(null, arg);
    } else if (arg.element) {
...
function geoPath() {
  ...
  if (arg['path-index']) {
    ...
    location['path-index'] = {text: args[0], namespaces: args[1]};
  } else if (valcheck.isString(arg)) {
    location['path-index'] = {text: arg, namespaces: ''};
  }
...
function bucket() {
  ...
  case 2:
    arg = arguments[1];
    var anchor = valcheck.isString(arg)             ? null : arg.anchor;
    var value  = valcheck.isNullOrUndefined(anchor) ? null : arg.value;
  ...
  case 4:
    arg = arguments[1];
    var anchor1    = valcheck.isString(arg)              ? null : arg.anchor;
    var value1     = valcheck.isNullOrUndefined(anchor1) ? arg  : arg.value[0];
    var comparator = arguments[2];
    arg = arguments[3];
    var anchor2    = valcheck.isString(arg)              ? null : arg.anchor;
    var value2     = valcheck.isNullOrUndefined(anchor2) ? arg  : arg.value[0];
