...
function addIndex(query, index, isContainer) {
  const containerOnly = (isContainer || false);
...
// TODO: elementName
  const maxIndex = Math.min(argLen, 2);
  let elemName = null;
  let arg = null;
  let i=0;
  while (i < maxIndex) {
...
function geoPropertyPair() {
  ...
  const keys = ['parent-property', 'lat-property', 'lon-property'];
  let key = null;
  let arg = null;
  for (let i=0; i < keys.length; i++) {
    key = keys[i];
...
function geospatialImpl(hasOperator, args) {
  ...
  let constraintName = null;
  let suggestOptions = null;
  let seekingWeight = true;
  let seekingFragmentScope = true;
  let seekingGeoOption = true;
  let seekingRegion = true;
  let seekingGeoOperator = true;
  let firstKey = null;
  let arg = null;
  let i=0;
  for (; i < args.length; i++) {
...
function NearDef(args) {
  ...
  const query               = {};
  const subquery            = [];
  let seekingOrdered      = true;
  let seekingWeight       = true;
  let seekingDistance     = true;
  let seekingMinDistance  = true;
  let arg                 = null;
  for (let i=0; i < args.length; i++) {
    arg = args[i];
    if (seekingOrdered) {
...
function periodCompare() {
  ...
  let compareAxis1    = null;
  let compareOperator = null;
  let compareAxis2    = null;
  let temporalOptions = null;
  let arg = null;
...
function range() {
  ...
  for (let i=0; i < args.length; i++) {
    ...
    if ((datatype === null || operator === null) && (typeof arg === 'string' || arg instanceof String)) {
      const testType = (datatype === null) ? datatypes[arg.trim()] : null;
      if (testType != null) {
        datatype = testType;
      } else {
        const testComp = (operator === null) ? comparisons[arg.trim()] : null;
        if (testComp != null) {
          operator = testComp;
        } else if (values === null) {
          values = [arg];
  ...
  const constraint = new RangeConstraintDef(constraintName, rangeDef, suggestOptions);
...
function copyFromQueryBuilder(otherQueryBuilder) {
  ...
  if (otherQueryBuilder != null) {
    ...
    const isString = (typeof otherQueryBuilder === 'string' || otherQueryBuilder instanceof String);
...
function where() {
  /*jshint validthis:true */
  const self = (this instanceof QueryBuilder) ? this : new QueryBuilder();
...
function sort() {
  ...
  const firstIndex = asIndex(args[0]);
  // null is a legitimate value of score
  const isScore    = (firstIndex.score !== void 0);
...
function facet() {
  ...
  const facetWrapper = {name: constraintName};
  const constraint = {facet: true};
  if (constraintIndex.collection !== undefined) {
    ...
    const constraintKeys = Object.keys(constraintIndex);
    const constraintKeyLen = constraintKeys.length;
    for (let j=0; j < constraintKeyLen; j++) {
      const key = constraintKeys[j];
...
function calculateFunction() {
  ...
  const rootname = mlutil.rootname(moduleName);
  if (rootname === null) {
...
function makeBindings(variant, args) {
  ...
  const desc = isParse ? 'parsing query text' : 'defining suggestion sources';
...
function EmptyApplyDef(binding) {
  ...
  const valid = {'all-results': true, 'no-results': true};
...
function extract() {
  ...
  if (typeof arg === 'string' || arg instanceof String) {
    ...
    const namespaces = arg.namespaces;
  ...
  for (let i=0; i < argLen; i++) {
    ...
    if (typeof arg === 'string' || arg instanceof String) {
      ...
      if (builtin !== void 0) {
        ...
        const rootname = mlutil.rootname(arg);
...
function makeSearchBody(builtQuery) {
  ...
  if (builtQuery.search !== void 0) {
    ...
    const whereClause = builtQuery.whereClause;
    if (builtQuery.queryType === 'qbe') {
      search.$query = whereClause.$query;
      const format    = whereClause.$format;
      const validate  = whereClause.$validate;
      if (format != null) {
    ...
    const calculateClause = builtQuery.calculateClause;
    if (calculateClause != null) {
      ...
      const searchConstraintList = searchOptions.constraint;
      if (searchConstraintList == null) {
        ...
        for (let li=0; li < searchConstraintList.length; li++) {
          const lookupConstraint = searchConstraintList[li];
          constraintLookup[lookupConstraint.name] = lookupConstraint;
        }

        const calculateConstraints = calculateClause.constraint;
        for (let mi=0; mi < calculateConstraints.length; mi++) {
          const calculateConstraint = calculateConstraints[mi];
          const searchConstraint = constraintLookup[calculateConstraint.name];
          if (searchConstraint == null) {
            searchConstraintList.push(calculateConstraint);
          } else {
            const constraintChildKeys =
              Object.keys(calculateConstraint).filter(notNameFilter);
            for (let ki=0; ki < constraintChildKeys.length; ki++) {
              const constraintChildKey = constraintChildKeys[ki];
              const searchChild = searchConstraint[constraintChildKey];
              const searchType = (searchChild === null) ?
    ...
    let searchFlags = searchOptions['search-option'];
    if (searchFlags == null) {
    ...
    const orderByClause = builtQuery.orderByClause;
    if (orderByClause != null) {
      searchOptions['sort-order'] = orderByClause['sort-order'];

      const scoreOption = orderByClause.scoreOption;
    ...
    const sliceClause = builtQuery.sliceClause;
    if (sliceClause != null) {
      ...
      const transformResults = sliceClause['transform-results'];
      ...
      const extractResults = sliceClause['extract-document-data'];
    ...
    const withOptionsClause = builtQuery.withOptionsClause;
    if (withOptionsClause != null) {
      ...
      for (i=0; i < optionsKeys.length; i++) {
        ...
        const mapping = optionKeyMapping[key];
        if (mapping != null) {
          const value = withOptionsClause[key];
          if (value != null) {
            if (mapping === true) {
              switch(key) {
              case 'categories':
...
function makeParsedQuery(searchBody, parsedQuery) {
  ...
  const hasConstraints     = (constraintBindings != null);
  const termBinding        = parsedQuery.term;
  const hasTerm            = (termBinding != null);
