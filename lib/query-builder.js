...
function parseBindings() {
  ...
  for (var i=0; i < argLen; i++) {
    ...
    if (arg.name !== undefined) {
      ...
      parseBindings.constraint = constraints;
      continue;
    }
...
function queryFunction() {
  ...
  var argLen = args.length;
  if (argLen < 2) {
    throw new Error('query function without module name or binding');
  }
  ...
  if (!valcheck.isString(constraintName)) {
    throw new Error('query function without a binding to a constraint name');
  }
