...
class Builder extends bldrgen.PlanBuilder {
  constructor() {
    ...
    if(this.ctsString === void 0){
      this.ctsString = new CtsExpr();
    }
  }
...
class CtsExpr{
  ...
  /**
   * Allows users to provide string input to cts.point server function. See {@link http://docs.marklogic.com/cts.point}
   * @since 3.3.0
   * @param args - string argument for cts point query.
   * @returns { CtsPoint }
   */
  point(...args) {
    return new types.CtsPoint('cts', 'point', args);
  }

  /**
   * Allows users to provide string input to cts.polygon server function. See {@link http://docs.marklogic.com/cts.polygon}
   * @since 3.3.0
   * @param args - The vertices of the polygon, given in order. No edge may cover more than 180 degrees of either latitude or longitude. The polygon as a whole may not encompass both poles. These constraints are necessary to ensure an unambiguous interpretation of the polygon. There must be at least three vertices. The first vertex should be identical to the last vertex to close the polygon. vertexes.
   * @returns { CtsPolygon }
   */
  polygon(...args) {
    return new types.CtsPolygon('cts', 'polygon', args);
  }
}
