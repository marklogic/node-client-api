...
class ServiceCaller {
  constructor(client, serviceDeclaration, endpointDeclarations) {
    ...
    if(endpointDeclarations === null || (Array.isArray(endpointDeclarations) && endpointDeclarations.length > 0)) {
      throw new Error('endpointDeclarations cannot be null or empty.');
    }
    ...
    }, client.createProxy(client, serviceDeclaration)
  );
  }
...
class EndpointCaller {
  constructor(client, endpointDeclaration) {
    ...
    if (endpoint === void 0 || endpoint === null) {
      throw new Error('endpointDeclaration endpoint cannot be null.');
    }
    const regex = /^(.*[/])([^/]+)(.[^/.]+)$/;
    ...
    if(endpointDirectory == null || endpointExtension == null) {
      throw new Error('endpoint must contain endpointDirectory and extension');
    }
    this.functionName = endpointFunctionName;
    endpointDeclaration.functionName = this.functionName;

    const serviceDeclaration = mlutil.copyProperties(endpointDeclaration);
    serviceDeclaration.functionName = endpointFunctionName;

    this.$mlProxy = client.createProxy(client, serviceDeclaration).withFunction(serviceDeclaration, endpointExtension);
