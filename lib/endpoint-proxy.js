...
function getParamDeclarations(functiondef, functionDeclaration) {
  ...
  return paramDeclarations.filter(paramDeclaration => {
    if (paramDeclaration === void 0 || paramDeclaration === null) {
      ...
      if (functiondef._sessionParam !== null) {
        throw new Error('function with multiple session parameters: ' + functionDeclaration.functionName);
      }
      functiondef._sessionParam = new Paramdef(paramDeclaration);
      return false;
    }
    return true;
  });
    ...
    switch (datatype) {
      case 'boolean':
        if (jstype === null) {
          ...
          throw new Error(`optional $jsType ${jstype} can only be "boolean" or "string" for datatype ${datatype}`);
        }
        break;
      case 'date':
      case 'dayTimeDuration':
      ...
      case 'unsignedLong':
        if (jstype === null) {
          ...
          throw new Error(`optional $jsType ${jstype} can only be "string" for datatype ${datatype}`);
        }
        break;
      case 'dateTime':
        if (jstype === null) {
          ...
          throw new Error(`optional $jsType ${jstype} can only be "Date" or "string" for datatype ${datatype}`);
        }
        break;
      case 'float':
      case 'int':
      case 'unsignedInt':
        if (jstype === null) {
          ...
          throw new Error(`optional $jsType ${jstype} can only be "number" or "string" for datatype ${datatype}`);
        }
        break;
      case 'array':
      case 'binaryDocument':
      ...
      case 'xmlDocument':
        if (jstype !== null) {
          throw new Error('cannot specify a $jsType for datatype: ' + datatype);
        }
        break;
      case 'session':
        throw new Error('can only specify session for parameter, not for return');
      default:
        throw new Error('unknown datatype for return value: ' + datatype);
    }
    this._jstype = jstype;
  }
  getJsType() {
...
class Functiondef {
  constructor(functionDeclaration, moduleDirectory, moduleExtension) {
    ...
    if (outputMode === void 0 || outputMode === null) {
      this._outputMode = 'promise';
    } else {
      const lowerOutputMode = outputMode.toLowerCase();
      switch(lowerOutputMode) {
        case 'promise':
        case 'stream':
          this._outputMode = lowerOutputMode;
          break;
        default:
