...
/*
 * ⚠️  SECURITY NOTICE: HTTP Digest Authentication (RFC 2617/7616)
 * 
 * This module implements HTTP Digest Authentication which requires MD5 per RFC 2617.
 * MD5 usage is protocol-mandated and cannot be replaced without breaking compatibility.
 * 
 * Security Considerations:
 *   - Passwords are NOT stored - only hashed transiently for challenge-response
 *   - MD5 is used for digest calculation, not password storage
 *   - Deploy over HTTPS/TLS to protect digest exchange
 *   - Server-controlled nonces prevent replay attacks
 * 
 * References:
...
const www_authenticator = function(username,password,options)
{
  if (2 == arguments.length && toString.call(password) != '[object String]') {
    options= password;
    password= null;
  }
  const credentials= user_credentials(username,password);
  let cnonce;
  if (options) {
  ...
  const parse_header= function(www_authenticate)
  {
    function Authenticator()
    {
      ...
      const auth_parms= this.parms= parsed.parms;
      this.cnonce= cnonce;
