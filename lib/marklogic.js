...
function removeAllClient(collection) {
  if (!valcheck.isString(collection)) {
    throw new Error('must specify at least one collection for quick document remove');
  }

  return this.documents.removeAll.call(this.documents, {collections: collection});
}
MarkLogicClient.prototype.removeCollection = removeAllClient;
