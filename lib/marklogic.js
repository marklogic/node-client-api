 ...
 * the database
 * @param {enum} [authType=digest] - the authentication type of digest|basic|certificate|kerberos
 ...
 * required for SSL
 * @param {Buffer} [pfx] - the client certificate and private key as a single PKCS12 file
...
function initClient(client, inputParams) {
  ...
  if (connectionParams.authType.toUpperCase() !== 'NONE' &&
      connectionParams.authType.toUpperCase() !== 'KERBEROS' &&
      connectionParams.authType.toUpperCase() !== 'CERTIFICATE' && (
