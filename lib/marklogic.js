 ...
 * @param {string} [principal] - the name of the ticket-receiving principal for
...
function initClient(client, inputParams) {
  ...
  var keys = ['host', 'port', 'database', 'user', 'password', 'authType', 'principal'];
  ...
  if (connectionParams.authType.toUpperCase() === 'KERBEROS' &&
      connectionParams.principal == null
      ) {
    throw new Error('kerberos authentication must include a principal');
  }
