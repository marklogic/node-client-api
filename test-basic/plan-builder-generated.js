...
describe('plan builder', function() {
  describe('expression functions', function() {
    ...
    it('vec.vectorScore#3', function(done) {
        ...
        testPlan([p.xs.unsignedInt(1), p.xs.double(1.2), p.xs.double(1.2)], p.vec.vectorScore(p.col("1"), p.col("2"), 0.75))
          .then(function(response) {
            should(String(getResult(response).value).replace(/^ /, '')).equal('340909');
            done();
        }).catch(done);
    });
    it('vec.vectorScore#4', function(done) {
        ...
        testPlan([p.xs.unsignedInt(1), p.xs.double(1.2), p.xs.double(1.2), p.xs.double(1.2)], p.vec.vectorScore(p.col("1"), p.col("2"), 0.75, p.col("4")))
          .then(function(response) {
            should(String(getResult(response).value).replace(/^ /, '')).equal('307377');
            done();
        }).catch(done);
    });
