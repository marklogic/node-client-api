...
describe("Transform test with javascript", function () {
  ...
  it("should write the transform", function (done) {
    this.timeout(10000);
    fs.createReadStream(transformPath).pipe(
      concatStream({ encoding: "string" }, function (source) {
        dbAdmin.config.transforms
          .write(transformName, "javascript", source)
          .result(function (response) {
            done();
          }, done);
      }),
  ...
  it("should read the transform", function (done) {
    dbAdmin.config.transforms.read(transformName).result(function (source) {
  ...
  it("should list the transform", function (done) {
    dbAdmin.config.transforms.list().result(function (response) {
  ...
  it("should  query", function (done) {
    db.documents.query(q.where(q.word("title", "test"))).result(function (
      response,
    ) {
      //console.log(JSON.stringify(response, null, 4));
  ...
  it("should modify during query , slice without paging parameters, Bug 111", function (done) {
    db.documents
      .query(
        q.where(q.word("title", "transform")).slice(q.transform(transformName)),
      )
      .result(function (response) {
        //console.log(JSON.stringify(response, null, 4));
        response[0].content.should.have.property("timestamp");
